...
DECLARE
  ...
  v_matches           boolean;
BEGIN
  -- Fetch product basics (brand/category are denormalized via joins, not
  -- columns on tenant_products).
  SELECT
    tp.tenant_id,
...
CREATE OR REPLACE FUNCTION "catalog"."brands_embedding_queue"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
begin
  if app.sync_trigger_bypass_active() then
...
ALTER FUNCTION "catalog"."brands_embedding_queue"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "catalog"."categories_embedding_queue"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
begin
  if app.sync_trigger_bypass_active() then
...
ALTER FUNCTION "catalog"."categories_embedding_queue"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "catalog"."enqueue_embedding"("p_entity_type" "text", "p_entity_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
...
ALTER FUNCTION "catalog"."enqueue_embedding"("p_entity_type" "text", "p_entity_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "catalog"."product_embedding_queue"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  v_product_id uuid;
...
ALTER FUNCTION "catalog"."product_embedding_queue"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "catalog"."products_embedding_queue"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
begin
  if app.sync_trigger_bypass_active() then
...
ALTER FUNCTION "catalog"."products_embedding_queue"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "catalog"."products_search_doc_update"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
begin
  if app.sync_trigger_bypass_active() then
...
ALTER FUNCTION "catalog"."products_search_doc_update"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."find_seller_candidates_by_phone"("p_phone" "text") RETURNS TABLE("user_id" "uuid", "tenant_id" "uuid", "tenant_name" "text", "tenant_slug" "text", "role" "text", "location_ids" "uuid"[])
    LANGUAGE "plpgsql" SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION "public"."get_user_workspace"("p_user_id" "uuid") RETURNS TABLE("workspace_type" "text", "role" "text", "tenant_id" "uuid", "tenant_slug" "text", "tenant_name" "text", "buyer_id" "uuid", "location_ids" "uuid"[])
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'app'
    AS $$
DECLARE
...
CREATE TABLE IF NOT EXISTS "catalog"."brand_images" (
    ...
    "image_type" "text" DEFAULT 'logo'::"text" NOT NULL,
    "r2_original_key" "text",
    "r2_medium_key" "text",
    "r2_thumb_key" "text",
    "contributed_by_tenant_id" "uuid",
    "status" "text" DEFAULT 'approved'::"text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "created_by" "uuid",
    "updated_by" "uuid",
    "deleted_at" timestamp with time zone,
    CONSTRAINT "brand_images_image_type_check" CHECK (("image_type" = ANY (ARRAY['logo'::"text", 'banner'::"text"]))),
    CONSTRAINT "brand_images_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'approved'::"text", 'rejected'::"text"])))
);


ALTER TABLE "catalog"."brand_images" OWNER TO "postgres";
...
ALTER TABLE "catalog"."brands" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "catalog"."category_images" (
    ...
    "updated_by" "uuid",
    "deleted_at" timestamp with time zone,
    CONSTRAINT "category_images_image_type_check" CHECK (("image_type" = ANY (ARRAY['icon'::"text", 'banner'::"text"]))),
    CONSTRAINT "category_images_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'approved'::"text", 'rejected'::"text"])))
);


ALTER TABLE "catalog"."category_images" OWNER TO "postgres";
...
ALTER TABLE "catalog"."embedding_queue" OWNER TO "postgres";


CREATE SEQUENCE IF NOT EXISTS "catalog"."embedding_queue_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER SEQUENCE "catalog"."embedding_queue_id_seq" OWNER TO "postgres";


ALTER SEQUENCE "catalog"."embedding_queue_id_seq" OWNED BY "catalog"."embedding_queue"."id";
...
CREATE TABLE IF NOT EXISTS "catalog"."integration_types" (
    ...
    "connectivity_mode" "text" DEFAULT 'cloud'::"text" NOT NULL,
    "is_active" boolean DEFAULT true NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "created_by" "uuid",
    "updated_by" "uuid",
    CONSTRAINT "integration_types_connectivity_mode_check" CHECK (("connectivity_mode" = ANY (ARRAY['cloud'::"text", 'local'::"text"])))
);


ALTER TABLE "catalog"."integration_types" OWNER TO "postgres";
...
ALTER TABLE "catalog"."product_aliases" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "catalog"."product_images" (
    ...
    "updated_by" "uuid",
    "deleted_at" timestamp with time zone,
    CONSTRAINT "product_images_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'approved'::"text", 'rejected'::"text"])))
);
...
ALTER TABLE ONLY "catalog"."embedding_queue" ALTER COLUMN "id" SET DEFAULT "nextval"('"catalog"."embedding_queue_id_seq"'::"regclass");
...
ALTER TABLE ONLY "catalog"."brand_images"
...
ALTER TABLE ONLY "catalog"."category_images"
    ADD CONSTRAINT "category_images_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "catalog"."embedding_queue"
    ADD CONSTRAINT "embedding_queue_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "catalog"."integration_types"
    ADD CONSTRAINT "integration_types_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "catalog"."product_aliases"
...
ALTER TABLE ONLY "catalog"."products"
    ADD CONSTRAINT "products_brand_id_master_sku_key" UNIQUE ("brand_id", "master_sku");
...
CREATE UNIQUE INDEX "catalog_product_images_primary_idx" ON "catalog"."product_images" USING "btree" ("product_id") WHERE (("is_primary" = true) AND ("deleted_at" IS NULL));



CREATE INDEX "idx_brands_is_public" ON "catalog"."brands" USING "btree" ("is_public");



CREATE INDEX "idx_brands_origin_tenant_id" ON "catalog"."brands" USING "btree" ("origin_tenant_id");



CREATE INDEX "idx_brands_slug" ON "catalog"."brands" USING "btree" ("slug");



CREATE INDEX "idx_catalog_brand_images_brand_deleted" ON "catalog"."brand_images" USING "btree" ("brand_id", "deleted_at");



CREATE INDEX "idx_catalog_brands_deleted_at" ON "catalog"."brands" USING "btree" ("deleted_at");



CREATE INDEX "idx_catalog_categories_deleted_at" ON "catalog"."categories" USING "btree" ("deleted_at");



CREATE INDEX "idx_catalog_category_images_category_deleted" ON "catalog"."category_images" USING "btree" ("category_id", "deleted_at");



CREATE INDEX "idx_catalog_product_images_product_sort" ON "catalog"."product_images" USING "btree" ("product_id", "deleted_at", "sort_order");



CREATE INDEX "idx_catalog_products_deleted_at" ON "catalog"."products" USING "btree" ("deleted_at");



CREATE INDEX "idx_categories_is_public" ON "catalog"."categories" USING "btree" ("is_public");



CREATE INDEX "idx_categories_parent_id" ON "catalog"."categories" USING "btree" ("parent_id");
...
CREATE UNIQUE INDEX "idx_embedding_queue_pending" ON "catalog"."embedding_queue" USING "btree" ("entity_type", "entity_id") WHERE ("processed_at" IS NULL);



CREATE INDEX "idx_embedding_queue_unprocessed" ON "catalog"."embedding_queue" USING "btree" ("created_at") WHERE ("processed_at" IS NULL);



CREATE INDEX "idx_product_aliases_product_id" ON "catalog"."product_aliases" USING "btree" ("product_id");



CREATE INDEX "idx_products_brand_id" ON "catalog"."products" USING "btree" ("brand_id");
...
CREATE INDEX "idx_products_embedding" ON "catalog"."products" USING "hnsw" ("embedding" "public"."vector_cosine_ops");



CREATE INDEX "idx_products_is_public" ON "catalog"."products" USING "btree" ("is_public");



CREATE INDEX "idx_products_master_sku" ON "catalog"."products" USING "btree" ("master_sku");
...
CREATE OR REPLACE TRIGGER "push-estimate-to-zoho" AFTER INSERT ON "app"."estimates" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ytlusgmlqxuosifeapkz.supabase.co/functions/v1/push-estimate-to-zoho', 'POST', '{"Content-type":"application/json","x-push-secret":"gGM2DkCdSUNGGj9GQnI7qvN1JjVgwDAHnmOZfkBnRLb1tsWoMXUjr1Tgh63dPPEA"}', '{}', '5000');



CREATE OR REPLACE TRIGGER "push-order-to-zoho" AFTER INSERT ON "app"."orders" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ytlusgmlqxuosifeapkz.supabase.co/functions/v1/push-order-to-zoho', 'POST', '{"Content-type":"application/json","x-push-secret":"gGM2DkCdSUNGGj9GQnI7qvN1JjVgwDAHnmOZfkBnRLb1tsWoMXUjr1Tgh63dPPEA"}', '{}', '5000');
...
CREATE OR REPLACE TRIGGER "brands_embedding_queue" AFTER INSERT OR UPDATE OF "name", "description" ON "catalog"."brands" FOR EACH ROW EXECUTE FUNCTION "catalog"."brands_embedding_queue"();



CREATE OR REPLACE TRIGGER "brands_updated_at" BEFORE UPDATE ON "catalog"."brands" FOR EACH ROW EXECUTE FUNCTION "app"."set_updated_at"();



CREATE OR REPLACE TRIGGER "catalog_brand_images_updated_at" BEFORE UPDATE ON "catalog"."brand_images" FOR EACH ROW EXECUTE FUNCTION "app"."set_updated_at"();



CREATE OR REPLACE TRIGGER "catalog_category_images_updated_at" BEFORE UPDATE ON "catalog"."category_images" FOR EACH ROW EXECUTE FUNCTION "app"."set_updated_at"();



CREATE OR REPLACE TRIGGER "catalog_product_images_updated_at" BEFORE UPDATE ON "catalog"."product_images" FOR EACH ROW EXECUTE FUNCTION "app"."set_updated_at"();



CREATE OR REPLACE TRIGGER "categories_embedding_queue" AFTER INSERT OR UPDATE OF "name" ON "catalog"."categories" FOR EACH ROW EXECUTE FUNCTION "catalog"."categories_embedding_queue"();



CREATE OR REPLACE TRIGGER "categories_updated_at" BEFORE UPDATE ON "catalog"."categories" FOR EACH ROW EXECUTE FUNCTION "app"."set_updated_at"();



CREATE OR REPLACE TRIGGER "integration_types_updated_at" BEFORE UPDATE ON "catalog"."integration_types" FOR EACH ROW EXECUTE FUNCTION "app"."set_updated_at"();



CREATE OR REPLACE TRIGGER "products_embedding_queue" AFTER INSERT OR UPDATE OF "name", "master_sku", "description", "attributes", "category_id" ON "catalog"."products" FOR EACH ROW EXECUTE FUNCTION "catalog"."product_embedding_queue"();



CREATE OR REPLACE TRIGGER "products_search_doc_update" BEFORE INSERT OR UPDATE OF "name", "master_sku", "description" ON "catalog"."products" FOR EACH ROW EXECUTE FUNCTION "catalog"."products_search_doc_update"();



CREATE OR REPLACE TRIGGER "products_updated_at" BEFORE UPDATE ON "catalog"."products" FOR EACH ROW EXECUTE FUNCTION "app"."set_updated_at"();
...
ALTER TABLE ONLY "catalog"."brand_images"
    ADD CONSTRAINT "brand_images_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "catalog"."brands"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."brand_images"
    ADD CONSTRAINT "brand_images_contributed_by_tenant_id_fkey" FOREIGN KEY ("contributed_by_tenant_id") REFERENCES "app"."tenants"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."brand_images"
    ADD CONSTRAINT "brand_images_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."brand_images"
    ADD CONSTRAINT "brand_images_updated_by_fkey" FOREIGN KEY ("updated_by") REFERENCES "auth"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."categories"
    ADD CONSTRAINT "categories_parent_id_fkey" FOREIGN KEY ("parent_id") REFERENCES "catalog"."categories"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."category_images"
    ADD CONSTRAINT "category_images_category_id_fkey" FOREIGN KEY ("category_id") REFERENCES "catalog"."categories"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."category_images"
    ADD CONSTRAINT "category_images_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."category_images"
    ADD CONSTRAINT "category_images_updated_by_fkey" FOREIGN KEY ("updated_by") REFERENCES "auth"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."integration_types"
    ADD CONSTRAINT "integration_types_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id");



ALTER TABLE ONLY "catalog"."integration_types"
    ADD CONSTRAINT "integration_types_updated_by_fkey" FOREIGN KEY ("updated_by") REFERENCES "auth"."users"("id");



ALTER TABLE ONLY "catalog"."product_aliases"
    ADD CONSTRAINT "product_aliases_product_id_fkey" FOREIGN KEY ("product_id") REFERENCES "catalog"."products"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "catalog"."product_images"
    ADD CONSTRAINT "product_images_contributed_by_tenant_id_fkey" FOREIGN KEY ("contributed_by_tenant_id") REFERENCES "app"."tenants"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."product_images"
    ADD CONSTRAINT "product_images_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."product_images"
    ADD CONSTRAINT "product_images_product_id_fkey" FOREIGN KEY ("product_id") REFERENCES "catalog"."products"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."product_images"
    ADD CONSTRAINT "product_images_updated_by_fkey" FOREIGN KEY ("updated_by") REFERENCES "auth"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "catalog"."products"
    ADD CONSTRAINT "products_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "catalog"."brands"("id") ON DELETE RESTRICT;
...
CREATE POLICY "catalog_brand_images_platform_all" ON "catalog"."brand_images" USING ("app"."is_platform_admin"()) WITH CHECK ("app"."is_platform_admin"());



CREATE POLICY "catalog_brand_images_public_select" ON "catalog"."brand_images" FOR SELECT USING ((("deleted_at" IS NULL) AND ("status" = 'approved'::"text") AND (EXISTS ( SELECT 1
   FROM "catalog"."brands" "b"
  WHERE (("b"."id" = "brand_images"."brand_id") AND ("b"."is_public" = true) AND ("b"."deleted_at" IS NULL))))));



CREATE POLICY "catalog_brands_platform_all" ON "catalog"."brands" USING ("app"."is_platform_admin"()) WITH CHECK ("app"."is_platform_admin"());



CREATE POLICY "catalog_brands_public_select" ON "catalog"."brands" FOR SELECT USING ((("is_public" = true) AND ("deleted_at" IS NULL)));



CREATE POLICY "catalog_categories_platform_all" ON "catalog"."categories" USING ("app"."is_platform_admin"()) WITH CHECK ("app"."is_platform_admin"());



CREATE POLICY "catalog_categories_public_select" ON "catalog"."categories" FOR SELECT USING ((("is_public" = true) AND ("deleted_at" IS NULL)));



CREATE POLICY "catalog_category_images_platform_all" ON "catalog"."category_images" USING ("app"."is_platform_admin"()) WITH CHECK ("app"."is_platform_admin"());



CREATE POLICY "catalog_category_images_public_select" ON "catalog"."category_images" FOR SELECT USING ((("deleted_at" IS NULL) AND ("status" = 'approved'::"text") AND (EXISTS ( SELECT 1
   FROM "catalog"."categories" "c"
  WHERE (("c"."id" = "category_images"."category_id") AND ("c"."is_public" = true) AND ("c"."deleted_at" IS NULL))))));



CREATE POLICY "catalog_embedding_queue_platform_all" ON "catalog"."embedding_queue" USING ("app"."is_platform_admin"()) WITH CHECK ("app"."is_platform_admin"());



CREATE POLICY "catalog_product_aliases_platform_all" ON "catalog"."product_aliases" USING ("app"."is_platform_admin"()) WITH CHECK ("app"."is_platform_admin"());



CREATE POLICY "catalog_product_aliases_select" ON "catalog"."product_aliases" FOR SELECT USING ((("deleted_at" IS NULL) AND (EXISTS ( SELECT 1
   FROM "catalog"."products" "p"
  WHERE (("p"."id" = "product_aliases"."product_id") AND ("p"."is_public" = true) AND ("p"."deleted_at" IS NULL))))));



CREATE POLICY "catalog_product_images_platform_all" ON "catalog"."product_images" USING ("app"."is_platform_admin"()) WITH CHECK ("app"."is_platform_admin"());



CREATE POLICY "catalog_product_images_public_select" ON "catalog"."product_images" FOR SELECT USING ((("deleted_at" IS NULL) AND ("status" = 'approved'::"text") AND (EXISTS ( SELECT 1
   FROM "catalog"."products" "p"
  WHERE (("p"."id" = "product_images"."product_id") AND ("p"."is_public" = true) AND ("p"."deleted_at" IS NULL))))));



CREATE POLICY "catalog_products_platform_all" ON "catalog"."products" USING ("app"."is_platform_admin"()) WITH CHECK ("app"."is_platform_admin"());



CREATE POLICY "catalog_products_public_select" ON "catalog"."products" FOR SELECT USING ((("is_public" = true) AND ("deleted_at" IS NULL)));
...
CREATE POLICY "integration_types_select" ON "catalog"."integration_types" FOR SELECT TO "authenticated" USING (true);
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "app"."campaigns";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "app"."estimates";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "app"."integration_sync_jobs";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "app"."invoices";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "app"."orders";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "app"."tenant_integrations";
...
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "catalog" GRANT SELECT ON TABLES TO "anon";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "catalog" GRANT SELECT ON TABLES TO "authenticated";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "catalog" GRANT ALL ON TABLES TO "service_role";
