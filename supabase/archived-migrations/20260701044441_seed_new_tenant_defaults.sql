-- Seed new tenants with signup contact info, explicit off defaults, and a 50km routing threshold.

DROP FUNCTION IF EXISTS app.create_tenant_and_admin(uuid, text, text, text, text);
...
GRANT EXECUTE ON FUNCTION app.create_tenant_and_admin(uuid, text, text, text, text, text, text, text, jsonb)
  TO service_role;
