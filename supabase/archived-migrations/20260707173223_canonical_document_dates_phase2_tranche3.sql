DROP FUNCTION IF EXISTS app.estimate_convert_to_invoice(uuid, uuid, uuid, timestamptz);
...
CREATE OR REPLACE FUNCTION app.estimate_convert_to_invoice(
  ...
  p_invoice_number_override text DEFAULT NULL,
  p_qty_overrides jsonb DEFAULT '{}'::jsonb
)
RETURNS jsonb
LANGUAGE plpgsql
...
GRANT EXECUTE ON FUNCTION app.estimate_convert_to_invoice(uuid, uuid, uuid, uuid[], date, text, jsonb) TO authenticated, service_role;
