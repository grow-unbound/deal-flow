-- Additive change only: the WhatsApp dispatch worker needs the sending
-- tenant's id to resolve per-tenant WABA credentials (Embedded Signup) with
-- a fallback to the platform number when a tenant hasn't connected one.
-- app.prepare_whatsapp_message_for_send's "ready" branch didn't return
-- tenant_id at all, so the caller had no way to know whose credentials to
-- use. Every other branch/line reproduced unchanged from
-- 20260825004617_whatsapp_cap_deferral_not_failure.sql.
