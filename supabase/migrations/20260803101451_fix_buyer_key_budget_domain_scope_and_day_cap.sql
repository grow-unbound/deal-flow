-- Fix metrics_v4_buyer_key_budget_exceeded root cause. Two independent
-- changes to app._metrics_v4_refresh_claimed_periods:
--
-- 1. The buyer/product/location key-collection + budget-check block used to
--    run unconditionally, before the domain dispatch. Traced every read of
--    the resulting temp tables: only the 'commercial' branch reads
--    metrics_v4_buyer_ids/metrics_v4_buyer_period_keys/metrics_v4_product_ids/
--    metrics_v4_product_period_keys (metrics_tenant_period_summary,
--    metrics_buyer_period_summary, metrics_product_period_summary); only
--    'commercial' (metrics_location_now_summary) and 'inventory'
--    (metrics_location_period_summary, metrics_warehouse_period_summary)
--    read metrics_v4_location_keys. A 'buyer_app' tick was paying the full
--    cost -- and budget-exceeded risk -- of collecting buyer/product/
--    location keys it then never used at all (buyer_app only builds
--    metrics_campaign_period_summary/metrics_cohort_period_summary, both
--    period_keys-driven, not entity-keyed). Gated all three collection
--    blocks to the domain(s) that actually consume them.
-- 2. Even for 'commercial'/'inventory' (which do need this), the buyer-id
--    collection fans out per DIRTY DAY to every buyer who transacted
--    tenant-wide that day -- confirmed on real data: 4 dirty days for
--    tenant d601c35c-1a78-4506-a556-a82118d72893 pulled in 434 distinct
--    buyers (each ordinary day alone already carries 60-200+), blowing the
--    250-buyer budget from completely normal daily volume, no backfill or
--    traffic spike involved. Added a separate, tighter day subset
--    (metrics_v4_key_collection_days, capped to the single oldest dirty day)
--    used ONLY by the buyer/product/location EXISTS(...) fan-out subqueries.
--    metrics_v4_period_keys (used for period-summary date-range coverage)
--    still reads the full (up to 100-row) metrics_v4_dirty_days as before --
--    unaffected, since period summaries legitimately need every touched
--    day/week/month/quarter and don't carry this fan-out risk. A batch
--    spanning >1 dirty day for commercial/inventory now processes 1 day per
--    tick instead of failing outright.
--
-- Body below is otherwise a verbatim copy of the current live definition
-- (20260803092609_pass_domain_to_landing_kpis_tail_call.sql).
...
BEGIN
  ...
  TRUNCATE pg_temp.metrics_v4_dirty_days, pg_temp.metrics_v4_period_keys, pg_temp.metrics_v4_buyer_ids, pg_temp.metrics_v4_product_ids, pg_temp.metrics_v4_buyer_period_keys, pg_temp.metrics_v4_product_period_keys, pg_temp.metrics_v4_location_keys, pg_temp.metrics_v4_key_collection_days;
  ...
  LEFT JOIN LATERAL (
    ...
    FROM items JOIN app.estimates e ON e.id = items.estimate_id AND e.tenant_id = p_tenant_id AND e.deleted_at IS NULL AND (app.estimate_status_counts_as_demand(e.status))
      AND app.metric_day_ist(e.estimate_date, e.created_at) >= k.period_start AND app.metric_day_ist(e.estimate_date, e.created_at) < k.period_end_exclusive
  ) est ON true
  LEFT JOIN LATERAL (
    WITH items AS MATERIALIZED (
  ...
  WHERE s.tenant_id = p_tenant_id AND s.tenant_product_id = k.tenant_product_id AND s.grain = k.grain AND s.period_start = k.period_start AND s.deleted_at IS NULL
    AND NOT EXISTS (
      ...
      SELECT 1 FROM inv_items JOIN app.invoices i ON i.id = inv_items.invoice_id AND i.tenant_id = p_tenant_id AND i.deleted_at IS NULL AND app.invoice_status_gmv_included(i.status)
        AND app.metric_day_ist(i.invoice_date, i.created_at) >= k.period_start AND app.metric_day_ist(i.invoice_date, i.created_at) < k.period_end_exclusive
      UNION ALL
      SELECT 1 FROM est_items JOIN app.estimates e ON e.id = est_items.estimate_id AND e.tenant_id = p_tenant_id AND e.deleted_at IS NULL AND (app.estimate_status_counts_as_demand(e.status))
        AND app.metric_day_ist(e.estimate_date, e.created_at) >= k.period_start AND app.metric_day_ist(e.estimate_date, e.created_at) < k.period_end_exclusive
      UNION ALL
  ...
  v_rows := v_rows + app._metrics_v4_refresh_landing_kpis(p_tenant_id, p_domain => p_domain);
