...
--
-- app.orders.idx_orders_tenant_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- idx_orders_is_buyer_app (tenant_id, is_buyer_app_order), which fully covers every
-- query idx_orders_tenant_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_orders_tenant_id;
