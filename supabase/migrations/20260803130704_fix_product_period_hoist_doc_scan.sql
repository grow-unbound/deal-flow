-- Hoist the parent-document scan out of the per-product LATERAL in the
-- commercial branch's metrics_product_period_summary INSERT and its DELETE
-- guard.
--
-- The previous shape used `WITH items AS MATERIALIZED (... WHERE
-- ii.tenant_product_id = k.tenant_product_id)` inside a LATERAL correlated to
-- each product x grain key. That fixed the *item* side join order (see the
-- earlier migration's comment), but left the *parent* side -- the
-- tenant+date-range scan of invoices/estimates/orders -- re-executed once per
-- key. Profiled on tenant d601c35c with 100 products x 2 grains:
--
--   old: Execution 961ms, shared hit=625328
--        (Index Scan on invoices ... rows=2624 loops=188, buffers=447850)
--   new: Execution 132ms, shared hit=33749
--
-- Cost goes from O(products x period_rows) to O(period_rows + item_rows), so
-- the win grows with the refresh-key budget: at the configured 250-key cap the
-- old shape is ~2.4s for this one block alone, against a 5000ms hard
-- tick_wall_budget_ms ceiling (enforced by a CHECK constraint on
-- app.metrics_runtime_control, so it cannot be tuned around).
--
-- This is a pure rewrite of the same computation -- the aggregates still read
-- the raw source tables, so the self-healing property is preserved. Verified
-- equal against the old formulation across day/month/quarter grains for all
-- of the tenant's products: 540 rows, 0 mismatches on units/value/count/
-- buyer_count.
--
-- Body below is otherwise a verbatim copy of the current live definition
-- (20260803121617_pass_dirty_days_to_landing_kpis_call.sql).
