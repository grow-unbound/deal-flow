...
--
-- app.cohort_members.idx_cohort_members_cohort_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- cohort_members_pkey (cohort_id, buyer_id (PRIMARY KEY)), which fully covers every
-- query idx_cohort_members_cohort_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_cohort_members_cohort_id;
