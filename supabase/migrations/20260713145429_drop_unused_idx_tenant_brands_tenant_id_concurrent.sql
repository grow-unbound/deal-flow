...
--
-- app.tenant_brands.idx_tenant_brands_tenant_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- tenant_brands_tenant_id_master_brand_id_key (tenant_id, master_brand_id (UNIQUE)), which fully covers every
-- query idx_tenant_brands_tenant_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_tenant_brands_tenant_id;
