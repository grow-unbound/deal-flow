...
--
-- catalog.categories.idx_categories_parent_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- categories_parent_id_slug_key (parent_id, slug (UNIQUE)), which fully covers every
-- query idx_categories_parent_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS catalog.idx_categories_parent_id;
