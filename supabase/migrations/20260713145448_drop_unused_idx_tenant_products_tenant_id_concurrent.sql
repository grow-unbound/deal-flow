...
--
-- app.tenant_products.idx_tenant_products_tenant_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- tenant_products_tenant_id_internal_sku_key (tenant_id, internal_sku (UNIQUE)), which fully covers every
-- query idx_tenant_products_tenant_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_tenant_products_tenant_id;
