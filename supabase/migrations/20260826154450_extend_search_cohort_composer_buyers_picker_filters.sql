-- Extend search_cohort_composer_buyers with the quick/advanced picker filters used by
-- the Buyer search-overlay picker (Pricelist, CustomerGroup, Campaign, and CatalogComposer
-- forms). Adds:
--   p_quarter_start/p_prev_quarter_start -- current & previous quarter boundaries, computed
--                                           JS-side via getSellerLandingPeriodMeta('quarter')
--                                           and passed in; the RPC never derives these itself.
--   p_quick_filters   text[] -- OR'd toggle chips: has_dues, overdue, app_enabled,
--                               dormant_qtr, buying_qtr, enquire_no_sales, top20
--   p_status          text   -- advanced dropdown: active | dormant | inactive
--   p_buyer_app_filter text  -- advanced dropdown: enabled | not_enabled
--   p_outstanding_filter text -- advanced dropdown: has_dues | overdue
--   p_location_id     uuid   -- advanced dropdown: buyers with >=1 invoice at this location
--                               this quarter. Implemented as an un-correlated semi-join
--                               against app.invoices (location_buyer_ids CTE below), NOT a
--                               per-buyer correlated EXISTS -- the existing index
--                               idx_invoices_tenant_location_invoice_date(tenant_id,
--                               location_id, invoice_date) already leads with location_id,
--                               so this plans as one index scan + hash semi join, cost
--                               scales with matching invoices, not with eligible buyer count.
--   invoice_count (returned) -- metrics_buyer_period_summary.invoice_count for the current
--                               quarter, shown alongside invoice_value in the picker row.
--
-- enquire_no_sales deliberately reads estimate_value/order_value, NOT primary_demand_value:
-- primary_demand_value/count are derived at write-time from app.metrics_v4_primary_demand_kind
-- and are hard-zeroed whenever a tenant's primary-demand module is configured 'none', even
-- with real estimate/order activity. metrics_product_period_summary has no primary_demand_value
-- column at all, so products must use estimate_value/order_value regardless -- using the same
-- two columns for buyers keeps both entities consistent and avoids the tenant-config gap.
--
-- top20 uses percent_rank() over the active-this-quarter universe (gmv_90d > 0), not
-- app.metrics_tenant_top80_cache (a different Pareto/revenue-contribution concept, not
-- "top 20% of buyer count"). Computed as a separate gmv_rank CTE left-joined onto filtered,
-- so buyers with zero quarter GMV simply have a NULL pct_rank and fail the top20 predicate.
--
-- Status='inactive': the base eligibility filter normally requires b.is_active = true (a
-- picker should default to only surfacing addressable buyers). When p_status = 'inactive' is
-- explicitly selected, eligibility flips to b.is_active = false instead, so deactivated
-- buyers become visible only when a seller deliberately asks for them.

DROP FUNCTION IF EXISTS app.search_cohort_composer_buyers(
  uuid, text, text[], text, text[], date, date, date, integer, integer, uuid[]
);

CREATE FUNCTION app.search_cohort_composer_buyers(
  p_tenant_id uuid,
    ...
    WHERE b.tenant_id = p_tenant_id
      ...
      AND (
        ...
        OR (
          ...
          AND (p_location_id IS NULL OR b.id IN (SELECT buyer_id FROM location_buyer_ids))
        )
      )
  ),
...
COMMENT ON FUNCTION app.search_cohort_composer_buyers(
  uuid, text, text[], text, text[], date, date, date, integer, integer, uuid[], date, date, text[], text, text, text, uuid
) IS 'Shared buyer-picker RPC for Cohort, Price List, Campaign, and CatalogComposer Add/Edit forms. v4-only sourcing (app.buyers, metrics_buyer_now_summary, metrics_buyer_period_summary), plus a location-scoped semi-join against app.invoices for the Sales Location filter. See picker filter definitions in src/lib/picker-filters.ts.';
