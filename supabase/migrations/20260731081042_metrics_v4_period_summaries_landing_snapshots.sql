...
--
-- app._metrics_v4_refresh_claimed_periods is the ONLY writer for every
-- period/now-summary table in this file, and is domain-scoped
-- (commercial -> tenant/buyer/product/brand/category, inventory ->
-- location/warehouse, buyer_app -> campaign/cohort, setup -> no-op, handled
-- by _metrics_v4_refresh_setup_now instead). Backfill/reconciliation always
-- goes through app.metrics_mark_reconciliation + app.metrics_refresh_tick
-- (claim/compute/acknowledge) so every write is bounded by the same
-- <=100-day / <=100-key budgets as steady-state ticks -- there is no
-- separate unbounded full-tenant-scan path. See
-- scripts/sql/metrics-v4-chunked-backfill.sql for a driver.
--
-- Explicit drop: an earlier version of this migration shipped these four as
-- permanently-disabled dead code (unconditional RAISE EXCEPTION guards).
-- Any environment that ran that version still has them on disk even though
-- this file no longer defines them -- CREATE OR REPLACE can't remove a
-- function it doesn't mention, so they need an explicit drop here.
DROP FUNCTION IF EXISTS app._metrics_v4_refresh_commercial_periods(uuid, timestamptz);
DROP FUNCTION IF EXISTS app._metrics_v4_refresh_entity_periods(uuid, timestamptz);
DROP FUNCTION IF EXISTS app._metrics_v4_refresh_campaign_periods(uuid, timestamptz);
DROP FUNCTION IF EXISTS app._metrics_v4_refresh_buyer_app_periods(uuid, timestamptz);
