...
--
-- app.order_items.idx_order_items_order_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- order_items_order_external_ref_upsert (order_id, external_ref (UNIQUE)), which fully covers every
-- query idx_order_items_order_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_order_items_order_id;
