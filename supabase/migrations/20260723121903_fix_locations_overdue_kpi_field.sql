-- Fixes the Locations landing "Overdue amount" KPI tile mislabel documented in
-- specs/kpi-callout-audit-2026-07-23.md §2 and §6 rule 9 (P0 item 1 in
-- specs/kpi-fix-execution-log.md).
--
-- Root cause: the tile was bound to `kpis.outstanding_dues_total`, which is
-- SUM(receivable_amount) — ALL outstanding unpaid invoices regardless of
-- whether due_date has passed. That's receivable, not overdue.
--
-- Fix: expose the already-computed `overdue_amount` (per location, sourced
-- from metrics_location_snapshot.overdue_amount, itself refreshed via
-- app.invoice_is_overdue) as its own summable KPI fields — `overdue_dues_total`
-- and `overdue_location_count` — without touching `outstanding_dues_total` /
-- `dues_location_count`, which may have other consumers. `overdue_location_count`
-- matches the existing "Collections overdue" callout's `overdue_amount > 0`
-- filter so the two numbers agree.
--
-- Full function body copied from
-- 20260723094535_fix_callout_limit_regression.sql (the current source of
-- truth) with only the `totals` CTE and the final `kpis` jsonb_build_object
-- changed.
