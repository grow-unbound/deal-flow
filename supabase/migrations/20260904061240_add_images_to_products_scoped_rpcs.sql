-- Add image columns to search_products_scoped/load_products_scoped so the
-- one enrichment round-trip also covers photos, instead of the JS layer
-- doing a second fetch for image_urls/r2 keys/brand logo/category image.
-- The brand (tb) and category (tc) tables are already joined into both
-- functions for name resolution -- this just selects a few more columns off
-- joins that already exist, no new joins.
--
-- RETURNS TABLE changes require DROP + CREATE (CREATE OR REPLACE cannot
-- change the return signature). Same body otherwise -- only added output
-- columns, no behavior change to any existing column.
