-- Fix: "value created"/demand flow metrics (estimate_value, app_estimate_value,
-- primary_demand_value, and the buyer/location/campaign demand-existence
-- checks) were computed with
-- `app.estimate_status_is_open(e.status) OR e.status = 'accepted'` (only
-- draft/sent/accepted counted). That silently dropped declined/expired/
-- invoiced/converted estimates -- real demand created in the period -- out of
-- the total. Per specs/metrics-v4-final-2026-07.md these are flow metrics:
-- date-windowed, no status filter beyond excluding dead/void records. Swapped
-- to `app.estimate_status_counts_as_demand(e.status)` (added in
-- 20260803080607_add_estimate_status_counts_as_demand_predicate.sql), which
-- only excludes 'void' -- the estimate-side equivalent of an order's
-- 'cancelled'. app.order_status_in_flow is untouched -- it already reduces to
-- "not cancelled" among real order statuses.
--
-- Body below is otherwise a verbatim copy of the current live definition
-- (20260802080330_metrics_v4_fix_tenant_period_value_fanout.sql) -- only the
-- predicate substitution described above was applied, nothing else changed.
