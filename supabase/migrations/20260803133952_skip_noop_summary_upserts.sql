-- Skip summary upserts that would write identical values.
--
-- Every ON CONFLICT ... DO UPDATE in the tick fired unconditionally, so each
-- 15s run rewrote the same rows even when nothing about them had changed.
-- Measured before this migration:
--
--   metrics_product_period_summary   419,081 updates /  2,122 live rows
--   metrics_buyer_period_summary     357,063 updates / 13,985 live rows
--   metrics_buyer_now_summary        179,502 updates /  5,062 live rows
--   metrics_category_period_summary   62,910 updates /    160 live rows
--   metrics_brand_period_summary      33,774 updates /    110 live rows
--
-- Each of those is a dead tuple, WAL, and autovacuum work for no information
-- gain. Adding a WHERE to DO UPDATE makes the upsert a genuine no-op when the
-- computed values match what is already stored.
--
-- What is compared, and why:
--   * the measure columns -- the actual payload
--   * source_watermark    -- deliberately INCLUDED. It is a real freshness
--                            signal read by metrics_refresh_tick's acknowledge
--                            stage; excluding it from the comparison would let
--                            it silently lag whenever measures happened to be
--                            unchanged. Including it means a source row being
--                            touched still advances the watermark, and only a
--                            genuinely identical recomputation is skipped.
--
-- What is NOT compared, because these change on every single execution by
-- construction and would defeat the guard entirely:
--   computed_at, updated_at, generation_id
--
-- period_end_exclusive is included for the period-grained tables: it is
-- functionally determined by (grain, period_start) so it should never differ,
-- but if it ever did that would be a correctness bug worth propagating rather
-- than skipping.
--
-- Body below is otherwise a verbatim copy of the current live definition
-- (20260803131547_bound_now_summary_lifetime_scans.sql).
