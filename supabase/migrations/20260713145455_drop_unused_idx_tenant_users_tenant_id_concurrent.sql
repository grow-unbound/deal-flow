...
--
-- app.tenant_users.idx_tenant_users_tenant_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- idx_tenant_users_tenant_user_active (tenant_id, user_id, is_active), which fully covers every
-- query idx_tenant_users_tenant_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_tenant_users_tenant_id;
