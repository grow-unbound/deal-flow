-- Fix: Estimates "Expiring in 7 days" card filtered
-- `app.estimate_status_is_open(e.status)` (draft/sent only). Per
-- specs/metrics-v4-final-2026-07.md, this card should include accepted
-- estimates too (draft, sent, accepted) -- an accepted estimate approaching
-- its valid_until date is exactly the kind of thing this card exists to
-- surface. Single computation site, reused for both the this-month snapshot
-- and the period-selector materialization loop.
--
-- Body below is otherwise a verbatim copy of the current live definition
-- (20260731081042_metrics_v4_period_summaries_landing_snapshots.sql) -- only
-- the expiring-7d predicate was changed, nothing else.
