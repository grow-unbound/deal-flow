-- Add stock_visibility_enabled / block_order_on_oos to buyer_app tenant settings.
-- Both default false; block_order_on_oos is only meaningful (and UI-toggleable) when
-- stock_visibility_enabled is true, but stored as an independent key.
...
-- Backfill existing tenants: fill in the two new keys (defaulting to false) without
-- touching any existing buyer_app values, including rows where buyer_app is absent.
UPDATE app.tenant_settings
SET settings = app.jsonb_deep_merge(
  jsonb_build_object(
    'buyer_app', jsonb_build_object(
      'stock_visibility_enabled', false,
      'block_order_on_oos', false
...
updated_at = now()
WHERE (settings #> '{buyer_app,stock_visibility_enabled}') IS NULL
   OR (settings #> '{buyer_app,block_order_on_oos}') IS NULL;
