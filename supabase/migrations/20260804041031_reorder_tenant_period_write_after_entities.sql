-- Move the metrics_tenant_period_summary write to AFTER the buyer and product
-- writes in the commercial branch.
--
-- Step 3 of specs/metrics-v4-period-rollup-design-2026-08-04.md, and a strict
-- no-op on its own: this migration only changes statement ORDER. It is landed
-- separately from the derivation switch precisely so that, if any stored value
-- moves, it is unambiguous whether the reorder or the derivation caused it.
--
-- New order within IF p_domain = 'commercial':
--   buyer_period -> buyer_now -> location_now -> product_period (+ DELETE guard)
--   -> tenant_period -> brand -> category
--
-- Safe because nothing between the old and new position reads
-- metrics_tenant_period_summary: brand and category read
-- metrics_product_period_summary, buyer_now reads the buyer key temp table, and
-- the tail call to _metrics_v4_refresh_landing_kpis still runs after all of it.
--
-- Body below is otherwise a verbatim copy of the current live definition
-- (20260804040354_buyer_summary_split_app_demand_columns.sql).
