-- Reinstates `last_primary_demand_at` on app.search_cohort_buyers_detail and
-- app.search_catalog_buyers, the one column still missing after
-- 20260723071313_fix_selectable_membership_detail_tables_cohort_scd2.sql patched these
-- functions from their ACTUAL deployed bodies. Base bodies here are the current deployed
-- versions (verified via pg_get_functiondef against project hcpzbnmumbykdqveyjhr) plus this
-- one column -- not supabase/migrations/20260722151700_selectable_membership_detail_tables.sql,
-- which is stale relative to prod (see comment in the 071313 migration for why).
--
-- app/api/tenant/catalogs/[id]/buyers/route.ts already reads row.last_primary_demand_at
-- from search_catalog_buyers; that field has been coming back undefined until this lands.
...
CREATE OR REPLACE FUNCTION app.search_catalog_buyers(p_tenant_id uuid, p_catalog_id uuid, p_query text DEFAULT NULL::text, p_member text DEFAULT 'yes'::text, p_status text[] DEFAULT NULL::text[], p_last_sale text[] DEFAULT NULL::text[], p_sales_90d text[] DEFAULT NULL::text[], p_buyer_app text[] DEFAULT NULL::text[], p_sort text DEFAULT 'gmv_desc'::text, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
 RETURNS TABLE(buyer_id uuid, buyer_name text, city text, geography_label text, cohort_label text, opened_status text, demand_value numeric, demand_count bigint, last_opened_at timestamp with time zone, last_conversion_at timestamp with time zone, last_primary_demand_at timestamp with time zone, is_member boolean, buyer_app_status text, primary_demand_kind text, total_count bigint, opens_count bigint, converted_count bigint, attributed_gmv numeric)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO ''
 SET statement_timeout TO '10s'
