-- Wire the buyer/product/location key budget checks in
-- _metrics_v4_refresh_claimed_periods to app.metrics_runtime_control
-- (control_scope='global') instead of the hardcoded literal 100 that
-- was previously disconnected from that config entirely. This is the
-- single common budget the app CLAUDE.md workflow rules asked for -- no
-- per-tenant override. Companion migration also widened the runtime
-- control table's own CHECK constraint ceiling from 100 to 500 and set
-- the global row's max_refresh_keys_per_tick to 250 (informed by real
-- per-day distinct-buyer touch data: peak 196, p95 193 for the busiest
-- tenant on this project), and removed vestigial control_scope='tenant'
-- override rows that mirrored the global defaults and served no purpose.
