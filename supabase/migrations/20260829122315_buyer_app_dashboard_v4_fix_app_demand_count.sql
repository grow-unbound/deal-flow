-- Correctness bug found by diffing this function's output against the old
-- v2 RPC's output side-by-side on live data (both still existed, so a
-- direct comparison was possible before cutover): v_demand_count in the
-- tenant-wide fast path was set from
-- app.metrics_tenant_period_summary.primary_demand_count, which is the
-- TOTAL primary-demand document count (all orders/estimates), not the
-- APP-SOURCED count. Old v2 always counted only is_buyer_app_order/
-- is_buyer_app_estimate docs here. Result: average_demand_docs_per_
-- enabled_customer and the demand_cancellation_rate denominator were both
-- inflated ~10x on Wine Yard (321 app orders vs 3,347 total orders this
-- quarter) -- comparing 'metrics' arrays from both RPCs side by side is
-- what caught it.
--
-- Fix: use app_order_count/app_estimate_count (both already present on
-- metrics_tenant_period_summary, same table) instead of primary_demand_count.
