-- Root-cause fix for the tick_wall_budget_exceeded deadlock found while
-- reconciling the estimate-demand-predicate fix: app._metrics_v4_refresh_landing_kpis
-- was called unconditionally at the tail of EVERY tick, regardless of which
-- of the 3 claimed_periods domains (commercial/inventory/buyer_app) was
-- running, and fully recomputed all 14 landing pages every time -- measured
-- at 1.74s alone against a real tenant (11k buyers, 18k invoices), before
-- any domain-specific work even ran. A full 3-domain dirty cycle therefore
-- recomputed everything 3x redundantly.
--
-- This migration:
-- 1. Adds a p_domain parameter (default NULL = compute everything, so any
--    other/future caller is unaffected) and gates each page's computation +
--    upsert call to only the domain(s) that actually own its source
--    table(s) -- traced precisely against the claimed_periods domain
--    branches, not guessed. Pages spanning 2 domains (customer_groups,
--    locations) run on either domain's tick, never skipped. price_lists is
--    config-only (no dependency on any of the 3 domains) so it's pinned to
--    'commercial' alone purely to avoid 3x redundant execution -- zero
--    staleness risk since nothing there ever varies by domain.
-- 2. Removes the entire 'locations' page block from the period-selector
--    loop -- per specs/metrics-v4-final-2026-07.md, Locations has no
--    selector at all (Invoiced Sales is "This Month" fixed, Top 80% is
--    "Month" fixed), so its presence there was never correct, not just
--    inefficient (it was also the site of a literal duplicate-inline-
--    subquery bug: the same top80_locations ranking query evaluated twice
--    per loop iteration for the value vs entity_count args).
-- 3. Replaces the 3 live top-80%-of-revenue window-function rankings
--    (customers/quarter, brands/month, locations/month) with a point-read
--    from app.metrics_tenant_top80_cache, populated once daily by
--    app.metrics_v4_refresh_top80_daily() (see the migration that added
--    that table+function). Revenue concentration doesn't meaningfully shift
--    tick-to-tick; COALESCE'd to 0 if no cache row exists yet.
--
-- Body is otherwise a verbatim copy of the current live definition
-- (20260803080805_fix_estimates_expiring_7d_include_accepted.sql) -- every
-- existing query is unmodified, only wrapped in domain-gate IF blocks or
-- (for the 3 top80 vars) replaced with a cache read.
...
ALTER FUNCTION app._metrics_v4_refresh_landing_kpis(uuid, timestamptz, text) OWNER TO postgres;
REVOKE ALL ON FUNCTION app._metrics_v4_refresh_landing_kpis(uuid, timestamptz, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION app._metrics_v4_refresh_landing_kpis(uuid, timestamptz, text) TO service_role;
