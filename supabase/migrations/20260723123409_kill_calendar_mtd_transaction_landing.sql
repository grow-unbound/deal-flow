-- Kill calendar-MTD from app.metrics_v2_transaction_landing (P1 item 7,
-- audit doc specs/kpi-callout-audit-2026-07-23.md §6 rule 1 / §7).
--
-- All three headline-KPI branches of this RPC (invoices/orders/estimates)
-- previously computed a *calendar* month-to-date window
-- (v_month = date_trunc('month', p_as_of AT TIME ZONE 'Asia/Kolkata') .. v_today,
-- with v_prev_month/v_prev_end as the prior calendar month) even though the
-- row tables these KPI strips sit above are filtered by a completely
-- different, user-selectable period (today/week/last90/quarter/year via
-- getSellerLandingPeriodMeta/resolveSellerLandingPeriod). Standard #1
-- (locked 2026-07-23) mandates trailing-90-day headline KPIs everywhere,
-- no calendar-MTD exception -- calendar-month/GST-period reporting is
-- deferred to a future "Exports & Reports" section, not built now.
--
-- This migration replaces the calendar-month window with the same
-- half-open trailing-90d idiom already established elsewhere in this
-- codebase (app.get_seller_locations_landing_summary's
-- p_current_start/p_current_end_exclusive params,
-- app.get_seller_category_landing_summary_v2's matching window,
-- src/lib/server/seller-period.ts's `day - 89`):
--   v_current_start          := (p_as_of AT TIME ZONE 'Asia/Kolkata')::date - 89
--   v_current_end (exclusive) := (p_as_of AT TIME ZONE 'Asia/Kolkata')::date + 1
--   v_previous_start          := v_current_start - 90
--   v_previous_end (exclusive) := v_current_start
--
-- 'headline_period' in the returned jsonb changes from 'this_month' to
-- 'trailing_90_days' for all three kinds, matching the value already used
-- by app.metrics_v2_products_landing for the same field. No other query
-- logic changes -- this is a verbatim copy of the current function body
-- (supabase/migrations/20260723121905_fix_orders_open_value_field.sql)
-- with only the window computation and headline_period literal touched.
