-- Pass the actual claimed dirty days through to the now-scoped
-- app._metrics_v4_refresh_landing_kpis (see the migration adding its
-- p_dirty_days parameter), so the period-selector loop only recomputes
-- selector periods that overlap this tick's dirty data instead of all 7
-- every time.
--
-- Body below is otherwise a verbatim copy of the current live definition
-- (20260803101451_fix_buyer_key_budget_domain_scope_and_day_cap.sql) -- only
-- this one call site changed, nothing else.
