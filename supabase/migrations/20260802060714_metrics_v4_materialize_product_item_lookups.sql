-- Profiled the product_period_summary insert against a real busy tenant:
-- 11.8s, 4.66M buffer hits (EXPLAIN ANALYZE, see session notes). Root cause:
-- the LATERAL's date-range condition on the parent doc table (invoices/
-- estimates/orders) is correlated to the outer k.period_start/period_end,
-- so Postgres can't sample it at plan time and defaults to an overly
-- optimistic row estimate (observed: 3 estimated vs 2610 actual). That bad
-- estimate makes the planner drive from the parent doc table and re-scan
-- the WHOLE tenant+date-range window from scratch on every one of the
-- (hundreds of) product x grain loop iterations, instead of starting from
-- the highly selective tenant_product_id filter on the item table.
--
-- Fix: WITH ... AS MATERIALIZED wrapping the item-table filter forces
-- Postgres to evaluate that selective, uncorrelated filter first (now aided
-- by the composite (tenant_product_id, <doc>_id) indexes added this
-- session), then join up to the parent doc table on its primary key for
-- just those few matched items. Companion migration also added those
-- composite indexes and dropped the plain tenant_product_id-only ones they
-- superseded. Net effect measured on the same real workload: 11.8s -> 1.2s.
