...
--
-- catalog.products.idx_products_brand_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- products_brand_id_master_sku_key (brand_id, master_sku (UNIQUE)), which fully covers every
-- query idx_products_brand_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS catalog.idx_products_brand_id;
