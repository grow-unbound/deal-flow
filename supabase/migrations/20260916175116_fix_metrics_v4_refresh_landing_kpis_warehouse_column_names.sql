-- app._metrics_v4_refresh_landing_kpis referenced ws.tracked_skus / ws.stockout_skus
-- on app.metrics_warehouse_now_summary, but that table has never had those columns
-- (it has available_product_count / out_of_stock_product_count instead). Every
-- 'inventory' domain metrics tick for every tenant has been failing on this since
-- whenever metrics_warehouse_now_summary was last shaped this way -- confirmed via
-- live probe: "column ws.tracked_skus does not exist" while draining WineYard's
-- (tenant d601c35c-1a78-4506-a556-a82118d72893) backlog. Root cause of stale
-- metrics_tenant_now_summary / metrics_refresh_state showing "stale" for the
-- inventory (and by extension buyer_app/commercial, which share the tick queue
-- budget) domain platform-wide, not specific to this tenant's migration issue.
...
BEGIN
  ...
  SELECT COALESCE(SUM(ws.sellable_units),0), COALESCE(SUM(ws.available_product_count),0), COALESCE(SUM(ws.out_of_stock_product_count),0)
  INTO v_sellable_units, v_warehouse_skus, v_warehouse_oos
  FROM app.metrics_warehouse_now_summary ws WHERE ws.tenant_id = p_tenant_id AND ws.deleted_at IS NULL;
  SELECT COUNT(*)::bigint INTO v_warehouse_no_sales
  FROM app.metrics_warehouse_now_summary ws
