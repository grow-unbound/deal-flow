-- Adds last_invoice_date to app.metrics_buyer_now_summary so the cohort composer's
-- last-order-bucket filter (rule-builder facet + saved automatic-cohort membership
-- rules) can read a v4 column instead of the retired v2 app.metrics_buyer_snapshot
-- table. Mirrors the day-bucketing (app.metric_day_ist) and status-filtering
-- (app.invoice_status_gmv_included) conventions metrics_buyer_period_summary already
-- uses for invoice_count/invoice_value, rather than the old v1 pattern (no status
-- filter, raw COALESCE). Backfill for existing rows is deferred -- new/updated rows
-- populate on the next 15s refresh tick for buyers touched by an invoice write
-- ('commercial' domain).
