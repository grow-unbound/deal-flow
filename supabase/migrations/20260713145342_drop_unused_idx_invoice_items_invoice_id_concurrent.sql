...
--
-- app.invoice_items.idx_invoice_items_invoice_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- invoice_items_invoice_external_ref_upsert (invoice_id, external_ref (UNIQUE)), which fully covers every
-- query idx_invoice_items_invoice_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_invoice_items_invoice_id;
