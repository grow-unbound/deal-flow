-- cohort_members SCD2 fix: app.metrics_v2_customers_landing's per-customer cohort
-- name/count and cohort-sourced price-list attribution must only reflect a buyer's
-- currently-active cohort membership. Full function body reproduced verbatim from
-- supabase/migrations/20260718132539_customers_landing_bounded_callout_previews.sql with
-- only the two cohort_members joins patched (cm.valid_until IS NULL added).
...
CREATE OR REPLACE FUNCTION app.metrics_v2_customers_landing(
  ...
  p_full_callout text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
VOLATILE
...
DECLARE
  ...
  v_needs_call_limit integer := CASE WHEN p_full_callout = 'needs_call' THEN 2147483647 ELSE 2 END;
  v_win_back_limit integer := CASE WHEN p_full_callout = 'win_back' THEN 2147483647 ELSE 2 END;
  v_needs_call_total bigint := 0;
  v_win_back_total bigint := 0;
  v_total bigint := 0;
  v_active bigint := 0;
  v_dormant bigint := 0;
...
BEGIN
  ...
  JOIN app.cohorts c
    ON c.id = cm.cohort_id AND cm.valid_until IS NULL
  ...
  WITH candidate AS (
    ...
    JOIN app.cohort_members cm ON cm.cohort_id = c.id AND cm.valid_until IS NULL
    JOIN _metrics_v2_customers_base b ON b.id = cm.buyer_id
  ...
  SELECT COUNT(*)::bigint INTO v_needs_call_total
  FROM _metrics_v2_customers_base
  WHERE overdue_amount > 0 OR receivable_amount > credit_limit;

  SELECT COUNT(*)::bigint INTO v_win_back_total
  FROM _metrics_v2_customers_base
  WHERE is_active AND (invoice_count_90d + estimate_count_90d + order_count_90d) = 0
    AND (last_order_at IS NULL OR GREATEST(0, EXTRACT(DAY FROM (p_as_of - last_order_at))::int) > 90);
  ...
  RETURN jsonb_build_object(
    ...
    'callouts', jsonb_build_object(
      ...
      'needs_call_total', v_needs_call_total,
      'win_back', v_win_back,
      'win_back_total', v_win_back_total
    ),
    'buyers', v_rows,
