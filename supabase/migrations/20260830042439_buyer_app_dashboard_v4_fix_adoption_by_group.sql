-- Last known hot spot from the 2026-08-29 buyer-app v4 cutover log:
-- adoption_by_customer_group cost ~362ms (6086 buffer hits) on Wine Yard.
-- EXPLAIN showed the culprit is `JOIN app.buyers b ON b.id = cm.buyer_id`
-- -- 757 individual pkey lookups into buyers, 2271 buffer hits for just
-- that join (~3 pages/row). buyers is a wide table (billing_address,
-- shipping_address, geography, custom_fields jsonb columns), so even a
-- pkey-indexed single-row fetch touches multiple heap/TOAST pages -- this
-- query only needs one boolean column (buyer_app_enabled) from it.
--
-- Same fix pattern as the assisted-without-access join fixed in
-- 20260829121935: join against the SMALL, selective set of
-- buyer_app_enabled=true buyer ids (~315/11,910 on Wine Yard) instead of
-- fetching the full wide row for every cohort member.
...
BEGIN
  ...
  WITH enabled_buyer_ids_for_groups AS (
    SELECT id FROM app.buyers WHERE tenant_id = p_tenant_id AND buyer_app_enabled
  ), group_rows AS (
    SELECT c.id, c.name,
      COUNT(DISTINCT cm.buyer_id) FILTER (WHERE eb.id IS NOT NULL) AS enabled_buyers,
      COUNT(DISTINCT a.buyer_id) AS active_buyers
    FROM app.cohorts c
    JOIN app.cohort_members cm ON cm.cohort_id = c.id
    LEFT JOIN enabled_buyer_ids_for_groups eb ON eb.id = cm.buyer_id
