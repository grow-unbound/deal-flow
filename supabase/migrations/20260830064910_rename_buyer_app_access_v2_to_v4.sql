-- Closes the debt flagged when search_buyer_app_access_v2 was rewired off
-- the v2 snapshot tables (previous migration): it kept the v2 name and
-- *_90d response keys even though it now reads month-to-date v4 period
-- data, not a rolling 90d window. Per owner instruction: rename the
-- function to v4 and the JSON keys to *_mtd for clarity. This IS a
-- coordinated API change -- app/api/tenant/buyer-app/access/route.ts,
-- src/hooks/useBuyerAppAccess.ts, and
-- src/components/seller/buyer-app/ManageAccessClient.tsx are updated in
-- the same commit as this migration.
CREATE OR REPLACE FUNCTION app.search_buyer_app_access_v4(p_tenant_id uuid, p_query text DEFAULT NULL::text, p_segment text DEFAULT 'all'::text, p_last_ordered text DEFAULT 'all'::text, p_sort text DEFAULT 'business_name'::text, p_location_ids uuid[] DEFAULT NULL::uuid[], p_limit integer DEFAULT 50, p_offset integer DEFAULT 0, p_include_summary boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  enriched AS MATERIALIZED (
    SELECT
      ...
      GREATEST(COALESCE(mbp.invoice_value, 0) - (COALESCE(mbp.app_order_value, 0) + COALESCE(mbp.app_estimate_value, 0)), 0)::numeric AS offline_spend_mtd,
      COALESCE(mbp.invoice_value, 0)::numeric AS total_spend_mtd,
      (COALESCE(mbp.app_order_value, 0) + COALESCE(mbp.app_estimate_value, 0))::numeric AS app_gmv_mtd,
  ...
  page AS MATERIALIZED (
    ...
    ORDER BY
      ...
      CASE WHEN v_sort = 'app_gmv' THEN f.app_gmv_mtd END DESC NULLS LAST,
      CASE WHEN v_sort = 'offline_spend' THEN f.offline_spend_mtd END DESC NULLS LAST,
    ...
    'buyers', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'offline_spend_mtd', p.offline_spend_mtd,
          'total_spend_mtd', p.total_spend_mtd,
          'app_gmv_mtd', p.app_gmv_mtd,
          'is_suggested', p.is_suggested,
          'is_inactive', p.is_inactive
        )
        ORDER BY
          CASE WHEN v_query IS NOT NULL THEN p.search_rank END DESC NULLS LAST,
          CASE WHEN v_sort = 'app_gmv' THEN p.app_gmv_mtd END DESC NULLS LAST,
          CASE WHEN v_sort = 'offline_spend' THEN p.offline_spend_mtd END DESC NULLS LAST,
...
DROP FUNCTION IF EXISTS app.search_buyer_app_access_v2(uuid, text, text, text, text, uuid[], integer, integer, boolean);
