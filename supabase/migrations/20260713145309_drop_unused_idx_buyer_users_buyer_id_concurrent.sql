...
--
-- app.buyer_users.idx_buyer_users_buyer_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- buyer_users_buyer_id_user_id_key (buyer_id, user_id (UNIQUE)), which fully covers every
-- query idx_buyer_users_buyer_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_buyer_users_buyer_id;
