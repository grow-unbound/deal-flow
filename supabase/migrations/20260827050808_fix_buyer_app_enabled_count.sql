-- app._metrics_v4_refresh_setup_now computed enabled_buyer_count as
-- COUNT(DISTINCT bu.buyer_id) over app.buyer_users -- i.e. "has any user
-- login linked" -- instead of app.buyers.buyer_app_enabled ("has Buyer App
-- access turned on"). On tenants where buyer_users rows exist independent of
-- buyer_app_enabled (e.g. wine-yard-technologies) this inflated the
-- Buyer App dashboard's "Customers with app access" KPI (and the derived
-- "App access, no order this quarter" KPI) to roughly the full buyer count
-- instead of the actual enabled count. The buyer_app landing page's other
-- KPI ("266 customers can self-serve") already used the correct
-- buyer_app_enabled flag via a separate RPC, which is how the mismatch was
-- caught.
--
-- Only that one subquery changes; the rest of the function body is carried
-- over unchanged from 20260807070128_fix_price_lists_now_all_buyers_scan.sql.
