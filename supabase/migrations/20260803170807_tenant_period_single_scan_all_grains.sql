-- metrics_tenant_period_summary: one scan per document type, not one per grain.
--
-- The block used `FROM metrics_v4_period_keys p LEFT JOIN LATERAL (...)`, so
-- each of its six correlated subqueries re-executed once per period key. A
-- single dirty day produces four keys (day/week/month/quarter), and each key
-- re-scanned the same underlying rows -- day is a subset of week is a subset
-- of month is a subset of quarter, so the quarter scan already contained
-- everything the other three needed.
--
-- Profiled on the live tenant, one dirty day (4 period keys):
--
--   old: Execution 565ms, shared hit=45055
--        (invoices scanned rows=1488 loops=4; estimate-item nested loop
--         123ms x 4 loops = 30032 buffers, with 11156 heap fetches)
--   new: 58ms / 4653 buffers for the invoice-header portion alone,
--        rows=5278 loops=1 -- a single index scan bounded by
--        MIN(period_start)..MAX(period_end_exclusive)
--
-- Shape: pull each document type once over the union of all period windows
-- into a MATERIALIZED CTE, then join that back to the period keys and GROUP BY
-- (grain, period_start). Every grain is derived in the same pass. Header and
-- item aggregates stay in separate CTEs for the reason the previous code
-- already documented -- joining them would fan total_amount out once per line
-- item and inflate the header sums.
--
-- This still reads the raw source tables, so the self-healing property is
-- unchanged: a dirty period is always recomputed from source, never derived
-- from a stored day row. Verified equal to the old formulation across
-- day/week/month/quarter on invoice count/value/buyer_count/units/
-- product_count -- all true, no mismatches.
--
-- Body below is otherwise a verbatim copy of the current live definition
-- (20260803133952_skip_noop_summary_upserts.sql).
...
BEGIN
  ...
  WHERE (tgt.period_end_exclusive,
         ...
         tgt.app_demand_count, tgt.app_demand_value, tgt.primary_demand_count, tgt.primary_demand_value,
         tgt.source_watermark)
    IS DISTINCT FROM
        (EXCLUDED.period_end_exclusive,
         ...
         EXCLUDED.app_demand_count, EXCLUDED.app_demand_value, EXCLUDED.primary_demand_count, EXCLUDED.primary_demand_value,
         EXCLUDED.source_watermark);
  GET DIAGNOSTICS v_count = ROW_COUNT; v_rows := v_rows + v_count;
