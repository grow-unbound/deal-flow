-- Fix conversion_90d: numerator must be invoices that were converted FROM a demand doc
-- (estimate_id IS NOT NULL OR order_id IS NOT NULL), not all invoices.
-- Previous formula used invoice_count_90d (all invoices) → always 100% for busy locations.

-- ─── Re-backfill conversion_90d with corrected formula ───────────────────────
DO $$
DECLARE
...
BEGIN
  ...
  SET
    conversion_90d = ROUND(
      LEAST(
        COALESCE(conv.converted_cnt90, 0)::numeric
          / NULLIF(s.estimate_count_90d + s.order_count_90d, 0) * 100,
        100
      ), 2
    ),
    ...
    SELECT
      COUNT(*) FILTER (
        WHERE app.metric_day_ist(i.invoice_date, i.created_at) >= v_today - 89
          ...
          AND (i.estimate_id IS NOT NULL OR i.order_id IS NOT NULL)
      ) AS converted_cnt90
    FROM app.invoices i
...
-- ─── Replace refresh kernel with corrected conversion formula ─────────────────
...
BEGIN
  ...
  SELECT l.tenant_id, l.id, 'metrics:location:' || l.id::text,
    ...
    -- converted_cnt90: invoices that originated from an estimate or order (not direct invoices)
    ROUND(LEAST(COALESCE(inv.converted_cnt90, 0)::numeric / NULLIF(COALESCE(est.cnt90, 0) + COALESCE(ord.cnt90, 0), 0) * 100, 100), 2),
  ...
  LEFT JOIN LATERAL (
    SELECT
      ...
      -- Invoices converted from demand docs (estimate or order) — used as conversion numerator
      COUNT(*) FILTER (
        WHERE app.metric_day_ist(i.invoice_date, i.created_at) >= v_today - 89
          AND app.invoice_status_gmv_included(i.status)
          AND (i.estimate_id IS NOT NULL OR i.order_id IS NOT NULL)
