-- Measured cost after 20260829121421 shipped: live EXPLAIN ANALYZE on Wine
-- Yard, warm cache: 1544ms mean (old v2, pg_stat_statements) -> 517ms (v4
-- as first written). A real ~3x, not the ~24x the price_lists fix got --
-- two blocks kept "as before" turned out to have the same low-selectivity
-- buyers-table problem found in price_lists's active_buyers scan:
--   - assisted-without-access list: 188ms, Seq Scan on buyers filtered
--     "NOT buyer_app_enabled" -- ~97% of buyers match that (11,443/11,910
--     on Wine Yard), so it's a full-table scan in a NOT-filter costume.
--   - adoption_by_customer_group (cohort_members x buyers): 362ms, same
--     root shape but needs business_name per cohort member regardless of
--     which side is selective -- left as a known remaining hot spot, not
--     fixed here (flagged for a follow-up, not blocking this cutover).
--
-- This migration fixes the assisted-without-access list only: buyer_app_
-- enabled=true is the SELECTIVE side (~315/11,910 buyers on Wine Yard,
-- ~2-3%) -- anti-join against that small set instead of scanning buyers
-- WHERE NOT buyer_app_enabled.
