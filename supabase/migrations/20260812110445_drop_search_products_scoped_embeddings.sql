-- Remove pgvector/embedding usage from the product search query path.
-- Embeddings are only built for app.tenant_products, and no OPENAI_API_KEY /
-- EMBEDDING_PROVIDER is configured anywhere in this app, so p_query_embedding
-- has always been NULL in production — this drops dead latency, not dead code.
-- Postgres can't drop a parameter via CREATE OR REPLACE, so the old signature
-- must be dropped first; grants do not survive a DROP and must be re-issued.

DROP FUNCTION IF EXISTS app.search_products_scoped(
  uuid, text, uuid, uuid, integer, integer, public.vector, uuid[], uuid[], uuid[], uuid[], uuid[], text, text, boolean, uuid, uuid
);
...
REVOKE ALL ON FUNCTION app.search_products_scoped(
  uuid, text, uuid, uuid, integer, integer, uuid[], uuid[], uuid[], uuid[], uuid[], text, text, boolean, uuid, uuid
) FROM PUBLIC;
REVOKE ALL ON FUNCTION app.search_products_scoped(
  uuid, text, uuid, uuid, integer, integer, uuid[], uuid[], uuid[], uuid[], uuid[], text, text, boolean, uuid, uuid
) FROM anon;
...
GRANT EXECUTE ON FUNCTION app.search_products_scoped(
  uuid, text, uuid, uuid, integer, integer, uuid[], uuid[], uuid[], uuid[], uuid[], text, text, boolean, uuid, uuid
) TO service_role;
