-- Bound the two *_now_summary blocks, which scanned all-time
-- invoices/estimates/orders per entity on every 15s commercial tick.
--
-- Both blocks were already correctly scoped to the entities this tick touched
-- (metrics_v4_buyer_period_keys / metrics_v4_location_keys) -- that was never
-- the problem. The problem was that each entity's aggregate had no time bound
-- and no status bound in the WHERE: the status/balance predicates were applied
-- inside FILTER (...), so every fully-settled invoice in history was read and
-- then discarded. Cost grew forever with document history.
--
-- Profiled before this migration (metrics_location_now_summary, 9 locations):
--   Execution 124ms, shared hit=15135
--   (Index Scan on estimates ... rows=706 loops=9; invoices ... rows=2059 loops=9)
--
-- Pushing `outstanding_balance > 0` into the WHERE is free for the *money*
-- columns: app.invoice_status_has_receivable is defined as
--   COALESCE(p_outstanding_balance,0) > 0 AND status IN (...)
-- and app.invoice_is_overdue delegates to it, so both FILTER clauses already
-- required a positive balance. A NULL balance is excluded identically either
-- way. This lets the partial indexes below do the work instead of the heap.
--
-- It is NOT free for source_watermark, which is a MAX over the same rows and
-- now tracks only unsettled documents. That narrowing is intentional and is
-- documented at each call site: these rows hold only receivable/overdue/credit
-- figures, which a settled document cannot affect. If a future consumer needs
-- an all-documents watermark on these two tables, it must not be taken from
-- here.
--
-- The open-estimate/open-order counts get matching partial indexes;
-- app.estimate_status_is_open and app.order_status_is_open are both IMMUTABLE,
-- so they are legal in an index predicate.

CREATE INDEX IF NOT EXISTS invoices_now_buyer_receivable_idx
  ON app.invoices (tenant_id, buyer_id)
  INCLUDE (outstanding_balance, status, due_date)
  WHERE deleted_at IS NULL AND outstanding_balance > 0;

CREATE INDEX IF NOT EXISTS invoices_now_location_receivable_idx
  ON app.invoices (tenant_id, location_id)
  INCLUDE (outstanding_balance, status, due_date)
  WHERE deleted_at IS NULL AND outstanding_balance > 0;

CREATE INDEX IF NOT EXISTS estimates_now_location_open_idx
  ON app.estimates (tenant_id, location_id)
  WHERE deleted_at IS NULL AND app.estimate_status_is_open(status);

CREATE INDEX IF NOT EXISTS orders_now_location_open_idx
  ON app.orders (tenant_id, location_id)
  WHERE deleted_at IS NULL AND app.order_status_is_open(status);

-- Body below is otherwise a verbatim copy of the current live definition
-- (20260803130704_fix_product_period_hoist_doc_scan.sql); only the two
-- *_now_summary blocks changed.
