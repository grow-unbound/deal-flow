...
--
-- app.price_list_items.idx_price_list_items_price_list_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- price_list_items_price_list_id_tenant_product_id_min_qty_key (price_list_id, tenant_product_id, min_qty (UNIQUE)), which fully covers every
-- query idx_price_list_items_price_list_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_price_list_items_price_list_id;
