-- Fix buyer-app dashboard metrics per diligence pass:
--   1. customers_submitting_app_demand: union across BOTH orders.is_buyer_app_order
--      and estimates.is_buyer_app_estimate (previously only counted whichever table
--      was the tenant's "primary demand kind", missing buyers on the other channel).
--   2. repeat_app_customers: union of buyer-app-flagged orders + estimates + invoices
--      within the 90d horizon, >=2 total docs (previously counted only one doc kind).
--   3. valuable_assisted_customers_without_access: read live app.buyers.buyer_app_enabled
--      instead of the periodically-refreshed app.metrics_buyer_snapshot.buyer_app_enabled,
--      to match the live-read pattern used everywhere else in this function.
-- app_sourced_invoiced_sales_share and customers_with_access were verified correct and
-- are unchanged.
