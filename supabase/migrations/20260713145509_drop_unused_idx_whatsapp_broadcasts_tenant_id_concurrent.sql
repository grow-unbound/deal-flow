...
--
-- app.whatsapp_broadcasts.idx_whatsapp_broadcasts_tenant_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- idx_whatsapp_broadcasts_tenant_created (tenant_id, created_at), which fully covers every
-- query idx_whatsapp_broadcasts_tenant_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_whatsapp_broadcasts_tenant_id;
