...
--
-- app.price_list_assignments.idx_price_list_assignments_price_list_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- price_list_assignments_zoho_upsert (price_list_id, target_type, target_id, external_ref (UNIQUE)), which fully covers every
-- query idx_price_list_assignments_price_list_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_price_list_assignments_price_list_id;
