-- Pass this function's own p_domain through to the now domain-scoped
-- app._metrics_v4_refresh_landing_kpis (see the migration adding that
-- p_domain parameter) instead of relying on its default (compute
-- everything), so each domain's tick only recomputes the landing-KPI pages
-- that domain actually owns, not all 14 every time.
--
-- Body below is otherwise a verbatim copy of the current live definition
-- (20260803080706_fix_estimate_demand_predicate_claimed_periods.sql) -- only
