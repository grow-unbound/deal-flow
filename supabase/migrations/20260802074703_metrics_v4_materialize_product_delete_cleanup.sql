-- Same MATERIALIZED fix as 20260802060714, applied to the product_period_
-- summary DELETE...NOT EXISTS cleanup statement right after the insert --
-- it has the identical item-table -> parent-doc-table join shape (invoice_
-- items/estimate_items/order_items joined up to invoices/estimates/orders
-- with a LATERAL-correlated date range), so it hits the same bad-estimate/
-- wrong-join-order cost. Not yet profiled in isolation on this table (the
-- insert dominated observed cost so far), but flagged as a known-same-shape
-- risk once backlog volume is high enough to expose it -- fixing
-- proactively rather than waiting to profile it separately.
