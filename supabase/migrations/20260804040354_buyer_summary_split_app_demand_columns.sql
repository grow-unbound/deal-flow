-- Split metrics_buyer_period_summary's combined app_demand_* into per-document-type
-- columns.
--
-- Step 1 of specs/metrics-v4-period-rollup-design-2026-08-04.md. This is an
-- output no-op for every existing column -- it only adds new ones -- so it can
-- land and be verified independently of the derivation switch.
--
-- Why: metrics_tenant_period_summary carries app_estimate_buyer_count and
-- app_order_buyer_count SEPARATELY, but the buyer table stores only the summed
-- app_demand_count / app_demand_value. Deriving the tenant month/quarter rows
-- needs COUNT(*) FILTER (WHERE app_estimate_count > 0) and the order
-- equivalent, which the combined column cannot answer -- a buyer with an app
-- order but no app estimate is indistinguishable from the reverse.
--
-- The values are already computed: the est/ord LATERALs in the buyer upsert
-- select app_estimate_count/value and app_order_count/value and then add them
-- together on the way in. This just stores them before summing.
--
-- app_demand_count / app_demand_value are kept, still written as the sum, so
-- nothing reading them changes.
--
-- Historical rows get 0 from the DEFAULT until the step-2 backfill populates
-- them; do NOT derive app_*_buyer_count from these columns before that
-- backfill completes.

ALTER TABLE app.metrics_buyer_period_summary
  ADD COLUMN IF NOT EXISTS app_estimate_count bigint  NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS app_estimate_value numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS app_order_count    bigint  NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS app_order_value    numeric NOT NULL DEFAULT 0;

-- Body below is otherwise a verbatim copy of the current live definition
-- (20260803170807_tenant_period_single_scan_all_grains.sql); only the
-- metrics_buyer_period_summary upsert changed.
