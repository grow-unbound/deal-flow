-- KPI title/subtitle copy ('label', 'supporting_text') was hardcoded as
-- literal SQL string arguments inside app.metrics_v4_kpi() and every one of
-- its 80 call sites in app._metrics_v4_refresh_landing_kpis(). That meant
-- any copy wording change needed a migration + refresh, and it bloated the
-- app.metrics_landing_kpi_snapshot.kpis jsonb with static text that never
-- varies at runtime. Copy now lives entirely in the frontend, keyed by
-- kpi_id per page (src/lib/seller-landing-kpi-copy.ts). This migration drops
-- the two text params from metrics_v4_kpi() and strips 'label'/
-- 'supporting_text' from the emitted card jsonb; the numeric fields the
-- frontend actually needs (entity_count, document_count, secondary_value)
-- are unchanged.
--
-- app._metrics_v4_refresh_landing_kpis body below is otherwise a verbatim
-- copy of the current live definition
-- (20260803121327_scope_landing_kpis_loop_by_dirty_day_overlap.sql) -- the
-- only diff is every app.metrics_v4_kpi(...) call dropping its label
-- (2nd positional arg) and supporting_text (7th positional arg, of the old
-- 9-arg form) literals.
...
BEGIN
  ...
  v_rows := v_rows + app._metrics_v4_upsert_landing_kpis(p_tenant_id, 'buyer_app', 'this_month', v_period.period_start, jsonb_build_array(
    app.metrics_v4_kpi('customers_with_access', COALESCE(v_now_summary.enabled_buyer_count,0), COALESCE(v_now_summary.enabled_buyer_count,0), NULL, NULL, 'now', jsonb_build_object('buyer_app_enabled',true)),
    app.metrics_v4_kpi('customers_submitting_app_demand', COALESCE((SELECT app_estimate_buyer_count + app_order_buyer_count FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = 'month' AND period_start = v_period.period_start AND deleted_at IS NULL),0), COALESCE((SELECT app_estimate_buyer_count + app_order_buyer_count FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = 'month' AND period_start = v_period.period_start AND deleted_at IS NULL),0), NULL, NULL, 'month', jsonb_build_object('source','buyer_app','period','this_month')),
    app.metrics_v4_kpi('app_sourced_demand', COALESCE((SELECT app_estimate_value + app_order_value FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = 'month' AND period_start = v_period.period_start AND deleted_at IS NULL),0), COALESCE((SELECT app_estimate_buyer_count + app_order_buyer_count FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = 'month' AND period_start = v_period.period_start AND deleted_at IS NULL),0), COALESCE((SELECT app_estimate_count + app_order_count FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = 'month' AND period_start = v_period.period_start AND deleted_at IS NULL),0), NULL, 'month', jsonb_build_object('source','buyer_app','period','this_month')),
    app.metrics_v4_kpi('repeat_app_customers', COALESCE((SELECT COUNT(*) FROM app.metrics_buyer_period_summary WHERE tenant_id = p_tenant_id AND grain = 'quarter' AND period_start = v_quarter.period_start AND app_demand_count >= 2 AND deleted_at IS NULL),0), COALESCE((SELECT COUNT(*) FROM app.metrics_buyer_period_summary WHERE tenant_id = p_tenant_id AND grain = 'quarter' AND period_start = v_quarter.period_start AND app_demand_count >= 2 AND deleted_at IS NULL),0), NULL, NULL, 'quarter', jsonb_build_object('source','buyer_app','demand_count_gte',2,'period','this_quarter'))
  ), v_watermark);
  END IF;
  ...
  LOOP
    ...
    v_rows := v_rows + app._metrics_v4_upsert_landing_kpis(p_tenant_id, 'buyer_app', v_period.period_key, v_period.period_start, jsonb_build_array(
      app.metrics_v4_kpi('customers_with_access', COALESCE(v_now_summary.enabled_buyer_count,0), COALESCE(v_now_summary.enabled_buyer_count,0), NULL, NULL, 'now', jsonb_build_object('buyer_app_enabled',true)),
      app.metrics_v4_kpi('customers_submitting_app_demand', COALESCE((SELECT app_estimate_buyer_count + app_order_buyer_count FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = v_period.grain AND period_start = v_period.period_start AND deleted_at IS NULL),0), COALESCE((SELECT app_estimate_buyer_count + app_order_buyer_count FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = v_period.grain AND period_start = v_period.period_start AND deleted_at IS NULL),0), NULL, NULL, v_period.grain, jsonb_build_object('source','buyer_app','period',v_period.period_key)),
      app.metrics_v4_kpi('app_sourced_demand', COALESCE((SELECT app_estimate_value + app_order_value FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = v_period.grain AND period_start = v_period.period_start AND deleted_at IS NULL),0), COALESCE((SELECT app_estimate_buyer_count + app_order_buyer_count FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = v_period.grain AND period_start = v_period.period_start AND deleted_at IS NULL),0), COALESCE((SELECT app_estimate_count + app_order_count FROM app.metrics_tenant_period_summary WHERE tenant_id = p_tenant_id AND grain = v_period.grain AND period_start = v_period.period_start AND deleted_at IS NULL),0), NULL, v_period.grain, jsonb_build_object('source','buyer_app','period',v_period.period_key)),
      app.metrics_v4_kpi('repeat_app_customers', COALESCE((SELECT COUNT(*) FROM app.metrics_buyer_period_summary WHERE tenant_id = p_tenant_id AND grain = 'quarter' AND period_start = v_quarter.period_start AND app_demand_count >= 2 AND deleted_at IS NULL),0), COALESCE((SELECT COUNT(*) FROM app.metrics_buyer_period_summary WHERE tenant_id = p_tenant_id AND grain = 'quarter' AND period_start = v_quarter.period_start AND app_demand_count >= 2 AND deleted_at IS NULL),0), NULL, NULL, 'quarter', jsonb_build_object('source','buyer_app','demand_count_gte',2,'period','this_quarter'))
    ), v_watermark);
...
ALTER FUNCTION app.metrics_v4_kpi(text, numeric, bigint, bigint, numeric, text, jsonb) OWNER TO postgres;
