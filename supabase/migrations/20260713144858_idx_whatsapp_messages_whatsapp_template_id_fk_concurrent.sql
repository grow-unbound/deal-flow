...
-- app.whatsapp_messages. This table takes high write volume from WhatsApp
-- notification/broadcast sends; an uncovered FK forces a sequential scan
-- on the referenced side for every cascade check / join, and slows down
-- FK-column lookups used by the notification workflow.
--
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_whatsapp_messages_whatsapp_template_id ON app.whatsapp_messages (whatsapp_template_id);
