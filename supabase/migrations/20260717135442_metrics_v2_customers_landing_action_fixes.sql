-- Customers landing actions, per specs/metrics-product-strategy-proposal-2026-07.md:
-- doc specifies exactly 2 actions (Collect overdue, Win back inactive customers).
-- The prior implementation had 3 (needs_call, top_spenders, top_risers):
--   - top_spenders isn't in the doc at all — dropped.
--   - top_risers ranked by growth_pct (a demand-recovery signal), not the
--     91-365-day inactivity rule the doc specifies for "win back inactive" —
--     replaced with a real inactive-buyer ranking (longest inactive first,
--     tie-broken by prior-year trailing-90d value).
--   - needs_call (Collect overdue) was missing invoice_count and days_overdue
--     supporting fields — added from metrics_buyer_snapshot.oldest_due_at,
--     which is tenant+buyer keyed so it's available in both the tenant-wide
--     and location-scoped code paths.
