-- Add estimate_count_90d, estimate_value_90d, order_count_90d, order_value_90d, conversion_90d
-- to metrics_location_snapshot and update the refresh kernel to populate them.
--
-- conversion_90d = % of demand docs (estimates + orders-in-flow, 90d) that produced an invoice (90d).
-- Capped at 100 to handle multi-invoice scenarios (one estimate → multiple invoices).

ALTER TABLE app.metrics_location_snapshot
  ADD COLUMN IF NOT EXISTS estimate_count_90d bigint DEFAULT 0 NOT NULL,
  ADD COLUMN IF NOT EXISTS estimate_value_90d  numeric(14,2) DEFAULT 0 NOT NULL,
  ADD COLUMN IF NOT EXISTS order_count_90d     bigint DEFAULT 0 NOT NULL,
  ADD COLUMN IF NOT EXISTS order_value_90d     numeric(14,2) DEFAULT 0 NOT NULL,
  ADD COLUMN IF NOT EXISTS conversion_90d      numeric(5,2)  DEFAULT 0 NOT NULL;

-- ─── Backfill existing rows from live tables ──────────────────────────────────
-- Uses the same day helper and status guards as the refresh kernel so numbers
-- are consistent with what future incremental runs will produce.
DO $$
DECLARE
  v_today date := (clock_timestamp() AT TIME ZONE 'Asia/Kolkata')::date;
BEGIN
  UPDATE app.metrics_location_snapshot s
  SET
    estimate_count_90d = COALESCE(est.cnt90, 0),
    estimate_value_90d = COALESCE(est.value90, 0),
    order_count_90d    = COALESCE(ord.cnt90, 0),
    order_value_90d    = COALESCE(ord.value90, 0),
    conversion_90d     = ROUND(
      LEAST(
        s.invoice_count_90d::numeric / NULLIF(COALESCE(est.cnt90, 0) + COALESCE(ord.cnt90, 0), 0) * 100,
        100
      ), 2
    ),
  ...
  LEFT JOIN LATERAL (
    SELECT
      COUNT(*) FILTER (WHERE app.metric_day_ist(e.estimate_date, e.created_at) >= v_today - 89)                                   AS cnt90,
  ...
  LEFT JOIN LATERAL (
    SELECT
      COUNT(*) FILTER (WHERE app.metric_day_ist(o.order_date, o.created_at) >= v_today - 89 AND app.order_status_in_flow(o.status))                       AS cnt90,
  ...
  WHERE s.location_id = l.id AND s.deleted_at IS NULL;
END;
$$;

-- ─── Replace refresh kernel to populate new columns going forward ─────────────
...
BEGIN
  ...
  INSERT INTO app.metrics_location_snapshot (
    ...
    estimate_count_90d, estimate_value_90d,
    order_count_90d, order_value_90d,
    conversion_90d,
  ...
  SELECT l.tenant_id, l.id, 'metrics:location:' || l.id::text,
    ...
    COALESCE(est.cnt90, 0), COALESCE(est.value90, 0),
    COALESCE(ord.cnt90, 0), COALESCE(ord.value90, 0),
    ROUND(LEAST(COALESCE(inv.cnt90, 0)::numeric / NULLIF(COALESCE(est.cnt90, 0) + COALESCE(ord.cnt90, 0), 0) * 100, 100), 2),
  ...
  LEFT JOIN LATERAL (
    SELECT
      COUNT(*) FILTER (WHERE app.metric_day_ist(i.invoice_date, i.created_at) >= v_today - 89 AND app.invoice_status_gmv_included(i.status)) AS cnt90,
  ...
  ON CONFLICT (tenant_id, location_id) WHERE deleted_at IS NULL DO UPDATE SET
    ...
    invoice_value_90d      = EXCLUDED.invoice_value_90d,
    purchasing_buyers_90d  = EXCLUDED.purchasing_buyers_90d,
    estimate_count_90d     = EXCLUDED.estimate_count_90d,
    estimate_value_90d     = EXCLUDED.estimate_value_90d,
    order_count_90d        = EXCLUDED.order_count_90d,
    order_value_90d        = EXCLUDED.order_value_90d,
    conversion_90d         = EXCLUDED.conversion_90d,
    ...
    linked_warehouse_count = EXCLUDED.linked_warehouse_count,
  ...
  WHERE ROW(
    ...
    app.metrics_location_snapshot.purchasing_buyers_90d,
    app.metrics_location_snapshot.estimate_count_90d, app.metrics_location_snapshot.estimate_value_90d,
    app.metrics_location_snapshot.order_count_90d, app.metrics_location_snapshot.order_value_90d,
    app.metrics_location_snapshot.conversion_90d,
    app.metrics_location_snapshot.open_estimate_count, app.metrics_location_snapshot.open_estimate_value,
    app.metrics_location_snapshot.open_order_count, app.metrics_location_snapshot.open_order_value,
    app.metrics_location_snapshot.receivable_amount, app.metrics_location_snapshot.overdue_amount,
    app.metrics_location_snapshot.linked_warehouse_count, app.metrics_location_snapshot.stocked_product_count,
    app.metrics_location_snapshot.low_stock_product_count, app.metrics_location_snapshot.out_of_stock_product_count,
    app.metrics_location_snapshot.source_watermark
