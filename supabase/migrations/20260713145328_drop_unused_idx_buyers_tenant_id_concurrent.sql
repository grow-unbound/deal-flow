...
--
-- app.buyers.idx_buyers_tenant_id is a plain, non-unique, non-partial btree
-- index on a column set that is already a leading-column prefix of
-- buyers_tenant_id_external_ref_key (tenant_id, external_ref (UNIQUE)), which fully covers every
-- query idx_buyers_tenant_id could serve (same leading column(s), no WHERE
...
DROP INDEX CONCURRENTLY IF EXISTS app.idx_buyers_tenant_id;
