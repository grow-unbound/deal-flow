-- cohort_members SCD2 fix: coverage/uncovered-cohort counts in
-- get_seller_price_list_landing_aggregates must only count a buyer's currently-active
-- cohort membership, not historical (closed) rows. Full function body reproduced verbatim
-- from supabase/migrations/20260718131308_price_list_landing_kpi_callout_fix.sql with only
-- the two cohort_members joins patched (cm.valid_until IS NULL added).
