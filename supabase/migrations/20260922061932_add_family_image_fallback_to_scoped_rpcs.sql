-- Add the missing family-image fallback step to search_products_scoped /
-- load_products_scoped. Today these two functions go straight from a SKU's
-- own image_urls to the category photo -- a SKU that belongs to a product
-- family with no own uploaded photo (common: many variants share one
-- representative family photo) has never had a way to surface that family
-- photo here, only the dedicated family-grouped catalog/detail pages (which
-- query app.tenant_product_families directly) show it correctly. This
-- affects every caller of these RPCs: cart resolve (/api/buyer/products/
-- resolve), catalog SKU-list browse, product detail, home reco.
--
-- Purely additive: same pattern as 20260904061240_add_images_to_products_
-- scoped_rpcs.sql (RETURNS TABLE change requires DROP + CREATE). No behavior
-- change to any existing column, one new LEFT JOIN, four new output columns.

DROP FUNCTION IF EXISTS app.search_products_scoped(uuid, text, uuid, uuid, integer, integer, uuid[], uuid[], uuid[], uuid[], uuid[], text, text, boolean, uuid, uuid);
DROP FUNCTION IF EXISTS app.load_products_scoped(uuid, uuid, uuid, integer, integer, uuid[], uuid[], uuid[], uuid[], uuid[], text, text, boolean, uuid, uuid);

CREATE FUNCTION app.search_products_scoped(
  p_tenant_id uuid,
...
RETURNS TABLE(
  ...
  r2_large_key text,
  brand_logo_url text,
  category_image_thumb_key text,
  category_image_medium_key text,
  family_image_urls text[],
  family_r2_small_key text,
  family_r2_medium_key text,
  family_r2_large_key text
)
LANGUAGE sql
SECURITY DEFINER
...
AS $$
  ...
  scoped_products AS (
    SELECT
      ...
      tc.r2_image_thumb_key AS category_image_thumb_key,
      tc.r2_image_medium_key AS category_image_medium_key,
      pf.image_urls AS family_image_urls,
      pf.r2_small_key AS family_r2_small_key,
      pf.r2_medium_key AS family_r2_medium_key,
      pf.r2_large_key AS family_r2_large_key,
      CASE
        WHEN NULLIF(btrim(p_query), '') IS NULL THEN NULL
        ELSE lower(
          concat_ws(
    ...
    LEFT JOIN app.tenant_product_families pf
      ON pf.id = tp.product_family_id
  ...
  SELECT
    ...
    page.r2_large_key,
    page.brand_logo_url,
    page.category_image_thumb_key,
    page.category_image_medium_key,
    page.family_image_urls,
    page.family_r2_small_key,
    page.family_r2_medium_key,
    page.family_r2_large_key
  FROM paged page
  CROSS JOIN normalized n
...
REVOKE ALL ON FUNCTION app.search_products_scoped(uuid, text, uuid, uuid, integer, integer, uuid[], uuid[], uuid[], uuid[], uuid[], text, text, boolean, uuid, uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION app.search_products_scoped(uuid, text, uuid, uuid, integer, integer, uuid[], uuid[], uuid[], uuid[], uuid[], text, text, boolean, uuid, uuid) TO service_role;

CREATE FUNCTION app.load_products_scoped(
  p_tenant_id uuid,
  p_buyer_id uuid DEFAULT NULL,
  p_price_list_id uuid DEFAULT NULL,
  p_limit integer DEFAULT 20,
