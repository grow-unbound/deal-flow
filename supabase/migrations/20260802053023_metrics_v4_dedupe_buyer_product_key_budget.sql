-- Dedupe the buyer/product key budget check to count distinct entities, not
-- entity x grain rows. Previously the temp buyer/product key tables were
-- built directly as (entity_id, grain, period_start) with PK on all three,
-- so every entity that needed both a month-grain and quarter-grain row
-- counted as 2 units against the budget -- double-charging for a single
-- raw-table scan per entity, which is the actual cost. Now: collect
-- distinct entity ids first (one scan, one budget check), then expand into
-- both grains unconditionally once the entity count has already cleared
-- the budget. Companion to 20260802052950 (daily reconciliation watermark)
-- -- together these should bring the real per-claim peak back under the
-- existing 250 global budget for even the busiest tenant; see the backfill
-- run notes for the actual measured numbers.
