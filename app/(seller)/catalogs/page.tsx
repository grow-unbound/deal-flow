...
import { useTenantCatalogs, type CatalogLandingRow } from '@/hooks/useCatalogs';
...
function CatalogRowReason(catalog: CatalogLandingRow) {
  if (catalog.status.label === 'Draft') {
    return 'Draft · not yet shipped to cohort';
  }
  if (catalog.status.label === 'Ended') {
    return `Ended ${catalog.valid_until_label} · ${catalog.orders} orders`;
  }
...
function CatalogsLandingContent() {
  ...
  const { data, isLoading, isError } = useTenantCatalogs();
  ...
  const filtered = useMemo(() => {
    ...
    return catalogs
      ...
      .filter((catalog) => {
        ...
        return catalog.name.toLowerCase().includes(query) || catalog.cohort_name.toLowerCase().includes(query);
      })
      .sort((a, b) => {
