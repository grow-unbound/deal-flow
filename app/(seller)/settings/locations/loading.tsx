  ...
  return (
    <PageWrap className="space-y-6">
      <div className="flex items-end justify-between gap-6 border-b border-transparent px-0 pt-6 pb-0">
        ...
        <div className="h-9 w-32 animate-pulse rounded-md bg-cream-100" />
      </div>
      ...
      <section className="overflow-hidden rounded-xl border border-cream-100 bg-white shadow-xs">
        ...
        <div className="space-y-4 px-5 py-5">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-cream-100 bg-cream-50 px-4 py-3">
            <div className="h-4 w-36 animate-pulse rounded bg-cream-200" />
            <div className="h-8 w-28 animate-pulse rounded-md bg-cream-100" />
          </div>
          <div className="overflow-hidden rounded-lg border border-cream-100">
            <div className="grid grid-cols-6 gap-0 border-b border-cream-100 bg-cream-50 px-5 py-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-3 animate-pulse rounded bg-cream-200" />
              ))}
            </div>
            {Array.from({ length: 5 }).map((_, r) => (
              <div key={r} className="flex border-b border-cream-100 px-5 py-3 last:border-0">
                <div className="h-10 flex-1 animate-pulse rounded bg-cream-50" />
              </div>
            ))}
          </div>
        </div>
