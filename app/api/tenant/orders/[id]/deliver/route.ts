...
export async function PATCH(
  ...
  try {
    ...
    const parsed = DeliverSalesOrderBodySchema.safeParse(body);
    if (!parsed.success) {
    ...
    if (!['dispatched', 'partially_dispatched'].includes(order.status)) {
      return NextResponse.json({ error: 'Order must be dispatched before marking delivered.' }, { status: 400 });
    }
    ...
    if (updateError) {
      console.error('[PATCH deliver]', updateError);
    ...
    const { error: auditError } = await db.schema('app').from('audit_log').insert({
      ...
      diff: { status: 'delivered', notify_buyer: parsed.data.notify_buyer ?? false },
      ts: nowIso,
    });
    if (auditError) {
    ...
    console.error('[PATCH /api/tenant/orders/[id]/deliver]', error);
