...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const catalogIds = Array.from(new Set(orders.map((o) => o.catalog_id).filter((id): id is string => id !== null)));

    let itemCounts: OrderItemCountRow[] = [];
    let catalogNames: CatalogNameRow[] = [];
    ...
    catalogNames = (catalogsRes.data ?? []) as CatalogNameRow[];
