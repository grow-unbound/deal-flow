    ...
    <>
      <div>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-1)', margin: '14px 0 0', padding: '0 16px' }}>
          {[
            { id: 'orders', label: 'Orders', count: orders.length, active: true },
            { id: 'enquiries', label: 'Enquiries', count: enquiries.length, active: false },
            { id: 'invoices', label: 'Invoices', count: invoices.length, active: false },
          ].map(t => (
            <div key={t.id} style={{ paddingBottom: 10, paddingRight: 18, borderBottom: t.active ? '2px solid var(--teal-500)' : '2px solid transparent', marginBottom: -1 }}>
              <span style={{ fontSize: 14, fontWeight: t.active ? 600 : 400, color: t.active ? 'var(--teal-500)' : 'var(--cream-700)' }}>{t.label}</span>
              <span style={{ marginLeft: 6, fontSize: 12, fontFamily: 'var(--font-mono)', color: 'var(--cream-600)', background: 'var(--cream-200)', padding: '1px 6px', borderRadius: 100 }}>{t.count}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: '12px 16px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--cream-50)', border: '1px solid var(--border-2)', borderRadius: 10, padding: '10px 14px' }}>
            ...
            <span style={{ fontSize: 14, color: 'var(--cream-500)' }}>Search orders…</span>
          </div>
        </div>
