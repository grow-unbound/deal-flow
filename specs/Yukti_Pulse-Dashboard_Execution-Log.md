...
**Overall status:** P01 complete with follow-up visual/data refinement; P2A/P2B complete; P3 complete with lab/browser caveat; P5 complete with seller-admin summary-backed opportunity reads and scoped-assistant omission until a scoped aggregate exists.
...
| `P5` | Phase 5: useful without adoption | P01 | `complete` | 2026-09-25 12:56 IST — p5-opportunity-read-path-performance |
...
## 2026-09-25 12:56 IST — p5-opportunity-read-path-performance — P5

**Status:** complete

**Branch / commit / PR:** `feat/pulse-revised` / commit pending at log-write time / PR explicitly not requested

**Objective:** Finish P5 by replacing the slow Pulse Opportunities portfolio read with a bounded summary-table read path, and document why the previous precomputed-looking flow was still slow.

### Completed

- Replaced seller-admin `/api/tenant/pulse/opportunities` reads with bounded `app.metrics_buyer_period_summary` + `app.buyers` reads for the two Phase 5 insights:
  - `valuable_assisted_customers_without_access` from current-quarter buyer invoice summaries.
  - `previously_submitted_app_demand_now_inactive` from prior-quarter app-demand summaries plus current-quarter app-demand posture.
- Replaced seller-admin opportunity buyer slide-over reads with the same summary-backed helpers, so opening the panel no longer calls the broad Buyer App dashboard portfolio RPC.
- Removed the P5 Opportunities dependency on `app.get_buyer_app_dashboard_v4` for seller-admin scope.
- For seller-assistant location subsets, intentionally return an empty P5 Opportunities result until a trustworthy per-location buyer-period aggregate exists. This preserves location isolation and follows Section 2.8's "omit or defer" rule instead of falling back to raw-table request-time aggregation.
- Kept Contribution, Demand Signals, and UI layout/skeleton behavior unchanged.
...
- `src/tests/pulse-api.test.ts`
- `specs/Yukti_Pulse-Dashboard_Execution-Log.md`
- Database objects changed: none. No migration push, no production mutation, and no persistent remote data mutation.

### Verification and evidence

| Check | Command/evidence | Result |
|---|---|---|
| Focused tests | `pnpm exec vitest run src/tests/pulse-core.test.ts src/tests/pulse-api.test.ts src/tests/pulse-client.test.tsx --pool=threads` | Passed: 3 files, 20 tests. The run printed a non-blocking pnpm registry metadata fetch warning before executing tests. |
| Type-check | `npx tsc --noEmit` | Passed. |
| Static hygiene | `git diff --check` | Passed. |
| Production build | `npm run build` | Passed. `/pulse` remains dynamic, route size `244 B`, first load JS `476 kB`; Pulse API routes remain `1.28 kB` / `282 kB`. |
| Data reconciliation | API tests assert seller-admin opportunities come from bounded summary tables, preserve activation and quiet-customer rows, and keep scoped-assistant opportunities empty when no scoped summary read exists. | Passed. |
| Security/scoping | No client-supplied tenant id. Seller-admin summary reads use verified JWT tenant id. Scoped assistants no longer receive tenant-wide P5 opportunity rows and no longer invoke the slow broad portfolio path for Opportunities. | Passed by static review and tests. |
| Root-cause query plan | Verified linked project ref before linked reads: `hcpzbnmumbykdqveyjhr`. Existing `SELECT app.get_buyer_app_dashboard_v4(...)` samples from the prior P5 entry were `932.692 ms` and `1505.895 ms`, shared hit `45803`. Inspection showed the RPC assembled a full Buyer App dashboard portfolio, including raw invoice/order/estimate/activity scans and unrelated action/explore widgets. | Root cause confirmed: the slow function was not reading only `metrics_landing_kpi_snapshot`; it was a broad portfolio RPC outside the Demand Signals snapshot path. |
| Activation summary plan | `EXPLAIN (ANALYZE, BUFFERS)` bounded top-N current-quarter buyer summary CTE + buyer PK lookup on WineYard dev. | Execution `74.364 ms`, shared hit `2983`, planning `7.190 ms`; under the `<100 ms` aggregate-read target and `<300 ms` widget target. |
| Quiet-customer summary plan | `EXPLAIN (ANALYZE, BUFFERS)` bounded prior-quarter app-demand summary CTE + buyer/current-period lookup on WineYard dev. | Cold sample `294.561 ms`, shared hit `2948`; warm repeat `5.706 ms`, shared hit `2948`, planning `3.559 ms`. Under widget target; warm aggregate read under target. |
| Naive summary plan avoided | `EXPLAIN` of a direct joined activation summary read without top-N prefilter. | Execution `1344.493 ms`, shared hit `3225`; proved that "precomputed" is insufficient without a bounded/index-friendly access pattern. |
| Cache/navigation | Existing independent TanStack query boundaries remain unchanged. No `router.refresh()`, no hydration-forced refetch, no full-page invalidation. | Static review passed. |
| Visual/layout | No Pulse component, loading skeleton, CSS, or route layout changes in this continuation. Production build route size unchanged. | No new visual regression surface introduced. |
| CSP review | No analytics, script, connect, image, CSP header, unsafe-inline, or eval changes. | Not applicable. |

### Findings

- The performance issue was not `metrics_landing_kpi_snapshot`; that table backs Demand Signals and was already measured at sub-millisecond direct lookup in earlier entries.
- P5 Opportunities were slow because the route called `app.get_buyer_app_dashboard_v4`, which builds the whole Buyer App analytics portfolio. Even when some fields are precomputed, the function still performs broad joins/scans for unrelated widgets and location-scoped parity.
- Precomputed tables still need widget-shaped access paths. A naive join from `metrics_buyer_period_summary` to all tenant buyers was also slow; the bounded top-N summary-first pattern was the measured fix.
- No live per-location buyer-period summary was present in the inspected `yukti-dev` schema, so assistant-scoped P5 opportunities are omitted rather than inferred from tenant totals or raw live scans.

### Decisions made

- Kept P5 scoped to the two Phase 5 eligible insights rather than restoring the older conversion/browsing groups through the broad portfolio RPC.
- Did not create a new migration or index because the bounded seller-admin path meets the immediate release gate with existing summaries.
- Did not add a raw SQL RPC; the app-layer PostgREST reads use existing tables, explicit tenant filters, bounded candidate caps, and buyer PK lookups.
- Marked P5 complete because the selected unit's seller-admin opportunity boundary now meets the performance gate and scoped assistants safely omit unavailable aggregate-backed rows.

### Deferred / explicitly out of scope

- A future scoped seller-assistant opportunity read should add or reuse a per-location buyer-period summary/read model before showing location-filtered P5 opportunities.
- Authenticated browser Web Vital traces and field p75 remain uncaptured; production build and route-size evidence are recorded instead.
- P4 maturity-adaptive ordering and P6 decline recovery remain out of scope.

### Risks or blockers

- Quiet-customer cold plan was close to the `<300 ms` widget target. If p95 worsens on larger tenants, add a narrow partial index or aggregate-backed RPC for prior-period app-demand candidates.
- The summary-table path intentionally reports the bounded candidate set, not an all-tenant exact count. This keeps the widget fast and action-oriented but should not be reused for formal reporting.
- Scoped assistants temporarily see no P5 Opportunities until a scoped aggregate is available.

### Rollback notes

- Revert this continuation commit to restore the prior P5 implementation that called `app.get_buyer_app_dashboard_v4` for Opportunities. No database rollback required.
- If the change must be partially rolled back, restore only the two Pulse Opportunities routes to `loadPulsePortfolio` / `portfolioToPulseOpportunities` and keep the type/test updates aligned.

### Recommended next unit

- Unit: `P4`
- Entry gate satisfied: yes
- Evidence / remaining requirement: P01, P3, and P5 are complete. P4 may begin maturity-adaptive presentation using the stabilized independent Contribution, Opportunities, and Demand Signals boundaries.
