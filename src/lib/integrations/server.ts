...
export async function loadIntegrationsSettingsPayload(tenantId: string) {
  ...
  const [
    ...
    db.schema('app').from('integration_sync_jobs').select('id, tenant_integration_id, job_type, phase, status, progress, error_log, summary, started_at, completed_at, created_at').eq('tenant_id', tenantId).is('deleted_at', null).order('created_at', { ascending: false }),
