    ...
    <section className="mt-5 space-y-4">
      <article className="overflow-hidden rounded-[14px] border border-cream-300 bg-white">
        <div className="border-b border-cream-300 px-5 py-4">
          <h3 className="font-display text-[17px] text-cream-950">Base pricing context</h3>
          <p className="text-[13px] text-cream-700">Use these values as a baseline while editing cohort and price-list overrides.</p>
        </div>
        <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-cream-700">MRP</p>
            <p className="mt-2 font-display text-[30px] leading-none text-cream-950">{pricingSummary.mrp != null ? formatCurrency(pricingSummary.mrp, 'INR') : '—'}</p>
          </div>
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-cream-700">Base selling price</p>
            <p className="mt-2 font-display text-[30px] leading-none text-cream-950">{pricingSummary.base_selling_price != null ? formatCurrency(pricingSummary.base_selling_price, 'INR') : '—'}</p>
          </div>
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-cream-700">Cost price</p>
            <p className="mt-2 font-display text-[30px] leading-none text-cream-950">{pricingSummary.cost_price != null ? formatCurrency(pricingSummary.cost_price, 'INR') : '—'}</p>
          </div>
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-cream-700">Margin</p>
            <p className="mt-2 font-display text-[30px] leading-none text-cream-950">{pricingSummary.margin_pct != null ? `${pricingSummary.margin_pct}%` : '—'}</p>
          </div>
        </div>
      </article>
      ...
      <section className="overflow-hidden rounded-[14px] border border-cream-300 bg-white">
        <div className="border-b border-cream-300 px-5 py-4">
          <h3 className="font-display text-[17px] text-cream-950">Pricing &amp; cohorts</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-left">
            <thead className="border-b border-cream-300 bg-cream-100 text-[11px] uppercase tracking-[0.06em] text-cream-700">
              <tr>
            ...
            <tbody>
              {pricing.map((row) => {
                ...
                return (
                  <tr key={row.item_id} className="border-b border-cream-300 align-top last:border-b-0">
                    <td className="px-5 py-3 text-[13px] font-medium text-cream-900">{row.price_list_name}</td>
                    <td className="px-5 py-3 text-[13px] text-cream-800">{row.cohorts.join(', ')}</td>
                    <td className="px-5 py-3">
                      {isEditing ? (
                        ...
                        <p className="font-mono text-[13px] text-cream-900">{formatCurrency(row.effective_price, 'INR')}</p>
                      )}
                    </td>
                    <td className="px-5 py-3 text-[12px] text-cream-700">
                    ...
                    <td className="px-5 py-3 text-[12px] text-cream-700">{row.is_active ? 'Active' : 'Inactive'}</td>
              ...
              {pricing.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-5 py-8 text-center text-[13px] text-cream-700">No price-list overrides for this product yet.</td>
                </tr>
              ) : null}
            </tbody>
          </table>
