      ...
      <div>
        ...
        <LandingTable
          ...
          columns={[
            ...
            { label: 'Avg discount', align: 'right' as const, width: 140, className: 'px-5' },
            ...(isAdmin ? [{ label: 'Avg margin', align: 'right' as const, width: 140, className: 'px-5' }] : []),
          ]}
          tableMinWidth={isAdmin ? 1428 : 1240}
          ...
          {rows.map((row) => {
            ...
            return (
              <SelectableRow key={row.price_list_id} selected={isSelected}>
                ...
                <td className={cn('px-3 py-3 font-mono text-sm', isExpired ? 'text-cream-500 line-through' : 'text-cream-900')}>
                  {validity}
                </td>
                <td className="px-3 py-3">
