...
interface ProductActivityTimelineProps {
  activity: ProductDetailResponse['detail']['activity'];
}

export function ProductActivityTimeline({ activity }: ProductActivityTimelineProps) {
  const groups = activity.reduce<Record<string, ProductDetailResponse['detail']['activity']>>((acc, item) => {
  ...
  return (
    <section className="mt-5 rounded-[14px] border border-cream-300 bg-white p-5">
      ...
      <p className="mt-1 text-base text-cream-700">Chronological changes related to this product</p>
      <div className="mt-4 space-y-6">
            ...
            <div className="space-y-3">
              {items.map((item) => (
                <div key={item.id} className="flex gap-3 rounded-[10px] border border-cream-200 bg-cream-50 px-3 py-2.5">
                  ...
                  <div>
                    <p className="text-base font-medium text-cream-900">{item.summary}</p>
        ...
        {activity.length === 0 ? (
          <div className="rounded-[10px] border border-cream-200 bg-cream-50 px-5 py-8 text-center text-base text-cream-700">
