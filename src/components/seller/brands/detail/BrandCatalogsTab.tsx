      ...
      >
        {filtered.map((catalog) => (
          <tr key={catalog.campaign_id} className="border-b border-cream-300 bg-white transition-colors hover:bg-cream-50">
            <td className="px-3 py-3 text-base font-medium text-cream-900">{catalog.campaign_name}</td>
            <td className="px-3 py-3 text-cream-900">{catalog.cohort_name}</td>
            <td className="px-3 py-3 font-display text-md text-cream-950">{formatNumberValue(catalog.gmv, 'CURRENCY_THRESHOLD')}</td>
            <td className="px-3 py-3 text-cream-900">{catalog.orders}</td>
            ...
            <td className="px-3 py-3 text-cream-900">{new Date(catalog.sent_at).toLocaleDateString('en-IN')}</td>
          </tr>
        ))}
      </LandingTable>
