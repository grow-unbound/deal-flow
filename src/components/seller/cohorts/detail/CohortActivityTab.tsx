...
import type { CohortDetailActivityItem } from '@/hooks/useCohorts';

interface CohortActivityTabProps {
  activity: CohortDetailActivityItem[];
}

export function CohortActivityTab({ activity }: CohortActivityTabProps) {
  const groups = activity.reduce<Record<string, CohortDetailActivityItem[]>>((acc, item) => {
  ...
  return (
    <section className="mt-5 rounded-[14px] border border-cream-300 bg-white p-5">
      ...
      <p className="mt-1 text-[13px] text-cream-700">Catalog publishes, assignments, member updates, and rule changes</p>
