...
export function CustomerActivityTab({ activity }: CustomerActivityTabProps) {
  ...
  return (
    <section className="mt-5 rounded-[14px] border border-cream-300 bg-white p-5">
      ...
      <p className="mt-1 text-[13px] text-cream-700">Invoices, payments, credit adjustments, catalog views, and order events</p>
                ...
                <div key={item.id} className="flex items-start gap-3 rounded-[10px] border border-cream-200 bg-cream-50 px-3 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-[13px] font-medium text-cream-900">{item.title}</p>
                      {item.amount != null ? (
                        <p className="font-mono text-[12px] text-cream-700">{formatCurrency(item.amount)}</p>
                      ) : null}
                    </div>
