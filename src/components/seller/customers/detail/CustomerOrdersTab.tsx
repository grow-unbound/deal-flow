  ...
  return (
    <section className="mt-5 overflow-hidden rounded-[14px] border border-cream-300 bg-white">
      <div className="border-b border-cream-300 px-5 py-4">
        <h3 className="font-display text-lg text-cream-950">Orders</h3>
        <p className="text-base text-cream-700">All orders placed by this buyer</p>
      </div>
      ...
      >
        {orders.map((order) => (
          ...
          >
            <td className="px-5 py-3.5 font-mono text-sm text-teal-700">{order.order_number ?? order.id.slice(0, 8)}</td>
            ...
            <td className="px-5 py-3.5 text-right font-display text-md text-cream-950">{formatCurrency(order.gmv)}</td>
